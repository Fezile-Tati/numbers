...
class PlatformEntry:
    ...
    # status displays (``numbers setup``, ``hermes status``, the dashboard
    ...
    # Env vars this platform needs (for ``numbers setup`` display).
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # invocation -- including plain `numbers chat`, which never touches any
        ...
        # `numbers setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``numbers setup``/``gateway status``, channel
