...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Numbers commands.
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Numbers version."""
        from hermes_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Numbers default runtime
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Numbers itself changed. ``--stat`` limits output to the summary.
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        ``numbers skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hermes_home
        from hermes_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `numbers skills diff <name>` is a
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Numbers is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if arg == "queue":
                behavior = "Messages will be queued for the next turn while Numbers is busy."
            elif arg == "steer":
                behavior = "Messages will be steered into the current run (after the next tool call)."
            else:
                behavior = "Messages will interrupt the current run while Numbers is busy."
            return EphemeralReply(
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Numbers to the latest version.

        Spawns ``numbers update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``numbers update`` may trigger. Marker
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Numbers')}"
        ...
        # Spawn `numbers update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `numbers update --gateway`
