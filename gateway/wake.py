...
* Stateless request/response adapters (the API server,
  ...
  ``X-Numbers-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Numbers-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Numbers-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Numbers-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-Numbers-Session-Id is rejected (403) on an unauthenticated API "
