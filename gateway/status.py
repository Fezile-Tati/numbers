...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Numbers' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Numbers gateway lifecycle subcommand from a command line.
    ...
    ``--profile``/``-p`` selectors from anywhere in argv -- Numbers's
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Numbers-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Numbers gateway."""
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    the identity into ``gateway_state.json`` lets `numbers update` (and the
