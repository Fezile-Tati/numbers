...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # The Numbers Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
# configuring, extending, and troubleshooting Numbers itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Numbers, so disable requests for these names are ignored
...
#   <root>/.hermes/skills/   — Numbers-native location
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Numbers can discover and view
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Numbers metadata, VCS, virtualenv/dependency, cache, and skill
