    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `numbers update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Numbers owns auxiliary retry + provider/model fallback policy (the
    ...
    # Numbers sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Numbers control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Numbers triggers summarization.  Higher values delay
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Numbers-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Numbers plumbing (_reasoning_config
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``numbers auth
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Numbers auth store.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `numbers setup` or `numbers model` and enter a valid http(s) base URL."
        ) from exc
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``numbers config aux reset``)."""
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Numbers' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `numbers model` but the
    ...
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: numbers model to reconfigure, or remove "
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Numbers owns
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: numbers auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: numbers model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: numbers model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Numbers will call. A requested model covers a compressor
...
def _get_task_timeout(task: str, default: float = _DEFAULT_AUX_TIMEOUT) -> float:
    """Read timeout from auxiliary.{task}.timeout in config, falling back to auxiliary.timeout then *default*."""
    if not task:
        return default
    task_config = _get_auxiliary_task_config(task)
    ...
    if raw is None:
        try:
            ...
            if isinstance(config, dict):
                raw = config.get("auxiliary", {}).get("timeout")
        except Exception:
            pass
    if raw is not None:
        try:
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Numbers reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Numbers reasoning config for direct model calls
