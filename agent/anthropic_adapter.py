"""Anthropic Messages API adapter for Numbers.

Translates between Numbers's internal OpenAI-style message format and
...
# Numbers effort → Anthropic adaptive-thinking effort (output_config.effort).
...
# Anthropic's OAuth billing classifier fingerprints certain Numbers tool
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("X-Title", "Numbers")
    ...
    # from ``ANTHROPIC_API_KEY`` (Numbers loads that into the process env from
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Numbers", "Claude Code")
                text = text.replace("Numbers agent", "Claude Code")
        ...
        #      a) bare Numbers-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Numbers surfaces in its CLI. We
