...
def _hermes_root_path() -> Path:
    """Resolve the Numbers root dir (always the parent of any profile, never per-profile)."""
    try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Numbers path.

    Three categories are blocked:

      * Internal Numbers cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global Numbers
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Numbers root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Numbers cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Numbers credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Numbers MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Numbers MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in hermes_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Numbers real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Numbers real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Numbers read (see
...
#
# Numbers profiles are separate HERMES_HOME dirs under
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Numbers scope, or is inside
...
# regardless of which Numbers profile is active. It does NOT cover the
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    ...
    index where the inner Numbers-state portion starts. Returns ``None`` for
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Numbers state.
    ...
    Detection is path-shape-only — does not require any Numbers resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Numbers process never "
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Numbers state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Numbers "
