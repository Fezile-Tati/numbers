...
def _direct_messages_for_pre_compress_memory(messages: Any) -> list[dict[str, Any]]:
    ...
    receive one normalized host contract instead of having to infer Numbers
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `numbers setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Numbers "
        ...
        if aux_context < threshold:
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"Numbers's small-context floor and output reservation "
...
def compress_context(
    ...
    *,
    ...
    # Numbers' summarizer would only rewrite a local mirror without shrinking
    ...
    # The memory-provider context handoff below is intentionally Numbers-only:
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Numbers' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Numbers' transcript
    unchanged.
    """
    auto_mode = str(
    ...
    # way the Numbers path below does. An active cooldown means a recent
