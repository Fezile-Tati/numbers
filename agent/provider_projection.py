"""Fold an agent-as-provider's own activity back into Numbers' turn state.

Most providers are models: they ask Numbers to run a tool and Numbers runs it, so
...
time Numbers sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — Numbers would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on Numbers tool iterations, of which there are none.
