...
A host that mints one physical ``session_id`` per RESPONSE (Numbers Studio's
...
Numbers sends — ``prompt_cache_key`` on both OpenAI-wire transports, plus the
...
Numbers must not infer the logical conversation from the id's SYNTAX (that
...
``X-Numbers-Session-Key`` header, which branching deliberately does NOT key
...
def declared_conversation_scope(agent: Any) -> Optional[str]:
    ...
    Resolved from ``agent._gateway_session_key`` (the ``X-Numbers-Session-Key``
