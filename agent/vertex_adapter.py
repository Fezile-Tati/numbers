"""Vertex AI (Google Cloud) adapter for Numbers.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows Numbers to use Gemini models via Google Cloud with
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed Numbers at Vertex.
    ...
    Numbers up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only Numbers-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the Numbers-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
