...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `numbers update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``numbers update``. On first observation we seed ``last_run_at`` to "now"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Numbers' background skill CURATOR. This is an "
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `numbers update` only if "
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``numbers model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `numbers model` → auxiliary
