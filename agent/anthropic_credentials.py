...
3. ``~/.hermes/.anthropic_oauth.json`` (Numbers PKCE login)
...
#     singleton file, which other Numbers processes/profiles read with fresh
        ...
        payload = json.dumps(
            {
                ...
                "comment": (
                    ...
                    "never durably committed. Written by Numbers so sibling "
...
def claude_code_credentials_path() -> Path:
    ...
    This file is not profile-owned: every Numbers profile's credential pool
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Numbers notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Numbers historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Numbers)
    ...
    # 1. Numbers-managed OAuth/setup token env var
    ...
    # 5. Numbers credential_pool OAuth entry.
...
# ── Numbers-native PKCE OAuth flow ────────────────────────────────────────
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Numbers-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Numbers with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Numbers-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Numbers OAuth credentials: %s", e)
    return None
...
def _write_hermes_oauth_credentials(
    ...
    try:
        ...
        logger.error(
            "Failed to write refreshed Numbers OAuth credentials to %s: %s", oauth_file, e
        )
