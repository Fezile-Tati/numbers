...
row taxonomy is inferred from the atlas shape so Numbers can render both legacy
...
# rows (1536x1872), while older Numbers/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Numbers' activity state names. They are not always identical to the
...
# Legacy Numbers/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Numbers activity names -> accepted row-name aliases in descending
