...
* new gateway turns get a brief "Numbers is paused" reply instead of an
...
def _canonical_root() -> Path:
    """Fleet-wide Numbers root, even when this process is a profile gateway.
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Numbers is paused ({reason}). New work is on hold; "
            "run `hermes resume` to pick things back up."
        )
    return (
        "⏸️ Numbers is paused. New work is on hold; "
        "run `hermes resume` to pick things back up."
    )
