...
# un-throttled log fires on *every* selection. On Windows several Numbers
...
# readiness handshake ("Timed out connecting to Numbers backend after
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``numbers model`` / ``numbers auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Numbers process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``numbers auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Numbers
        ...
        # Previously "anthropic" was excluded from this tuple: two Numbers
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every Numbers profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every Numbers process) that might
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Numbers
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Numbers process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Numbers profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `numbers model` / `numbers auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `numbers model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``numbers auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `numbers auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `numbers auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Numbers PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `numbers setup` ("Claude
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `numbers auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (numbers auth remove copilot gh_cli) must not pay either on
        ...
        # Numbers auth store or env vars, so resolve them here.
        ...
        # standard `numbers auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `numbers auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Numbers auth store.  Without this gate the removal is instantly
        ...
        # Numbers owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``numbers model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``numbers auth list`` reflects the logged-in state and so the pool
...
# authoritative source for Numbers credentials. Stale env vars from parent
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `numbers auth remove <provider> <N>` for an
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `numbers auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Numbers
