...
(``gateway/platforms/webhook.py``): inbound wakes Numbers when the world
changes; outbound tells the world when Numbers does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Numbers-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
Headers::
    ...
    User-Agent:              Numbers-Agent-Outbound-Webhook
    X-Numbers-Event:          <hook event name>
    X-Numbers-Delivery:       <delivery_id>
    X-Numbers-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Numbers-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Numbers-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
