...
* The ``iron-proxy`` binary is auto-installed into ``<hermes_home>/bin/iron-proxy``
  on first use.  Numbers pins one upstream version (``_IRON_PROXY_VERSION``)
...
# credential (Numbers' auth.py keys Google on both GEMINI_API_KEY and
...
def write_proxy_config(config: Dict) -> Path:
    ...
    try:
        import yaml  # PyYAML is already a Numbers dep
...
def start_proxy(
    *,
    ...
    # close-on-success path, every restart leaked one fd in the Numbers
    # process.
    #
