...
Owns one Codex thread per Numbers session. Drives `turn/start`, consumes
...
# Numbers' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Numbers/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Numbers session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Numbers' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Numbers' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Numbers' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Numbers
        approval-mode/timeout logic. The Numbers-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Numbers approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Numbers approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Numbers returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Numbers approval mode/timeout resolution
    ...
    # "prompt expired"; the Numbers-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort Numbers version string for codex's userAgent line."""
    try:
