...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Numbers does
...
class EntraIdentityConfig:
    ...
    Captures the Numbers-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Numbers-selected knobs are passed as kwargs. Everything else
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Numbers processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    ``numbers auth status`` / wizard preflight.
