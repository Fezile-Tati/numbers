"""Upload a Numbers session transcript to Hugging Face as an agent trace.

Numbers stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Numbers' secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Numbers OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Numbers' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Numbers conversation messages as Claude Code JSONL text.
