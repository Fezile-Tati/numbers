...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches Numbers through a client
    shim, and a shim that cannot carry Numbers tool calls back turns the fork into
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Numbers, e.g. 'hermes-agent').\n"
    "  • Hub-installed skills (installed via 'numbers skills install').\n"
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry Numbers tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit Numbers tool calls, "
