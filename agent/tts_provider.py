...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Numbers with shell-template placeholders. **Wins over a
...
``{success: False, error: …}`` JSON envelope the rest of Numbers
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``numbers tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``numbers setup`` for
