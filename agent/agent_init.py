...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Numbers never even initiates the compaction), so no
...
def init_agent(
    ...
    # management API (the historical Numbers behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Numbers' built-in
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Numbers.  Choose a model with at least "
    ...
    # Nous Numbers 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            ...
            if _hermes_warn:
                _user_msg = (
                    "⚠ Nous Research Numbers 3 & 4 models are NOT agentic — they "
