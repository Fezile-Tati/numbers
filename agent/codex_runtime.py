...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Numbers accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Numbers' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Numbers should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Numbers state.

    The app-server owns the compacted thread context, so Numbers should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Numbers deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → Numbers UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Numbers tool loop. Without this bridge gateway
...
# Codex item types that map to a Numbers tool_call in the projector (and
...
# Internal MCP server that wraps Numbers' native tools for codex. When
...
# vision_analyze, ...) since the user thinks of these as Numbers tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "hermes-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Numbers tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Numbers' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into Numbers' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Numbers'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Numbers' standard prompt flow if a
        ...
        # explicitly opted out of Numbers approvals — via `approvals.mode: off`
        ...
        # with a missing Numbers UI. Defaults (manual/smart/unset) preserve the
        ...
        # item/agentMessage/delta, ...) into Numbers' gateway UI callbacks
...
def _is_plain_json_data(value: Any) -> bool:
    ...
    format.  Numbers assembles Codex payloads from JSON round-trips, so they
