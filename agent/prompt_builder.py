...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Numbers, built by Nous Research. Be direct: match the "
...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    "You run on Numbers (by Nous Research). When the user needs help with "
    "Numbers itself — configuring, setting up, using, extending, or troubleshooting "
    ...
    "modifying, or troubleshooting Numbers so you don't guess or invent workarounds."
)
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Numbers appends their message to the "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Numbers floating over another application, so the
    user is rarely asking about Numbers — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Numbers "
        ...
        "rather than anything inside Numbers. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Numbers terminal UI (TUI). Files: there is no "
    ...
    "desktop": (
        ...
        "You are chatting inside the Numbers desktop app, a graphical chat "
...
# container / remote host rather than on the machine where Numbers itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Numbers runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Numbers itself is running. The host OS, home, and cwd "
                f"of the Numbers process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Numbers "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Numbers
...
def build_context_files_prompt(
    ...
    # Never let a FALLBACK-picked directory inside the Numbers install/source
    ...
    # explicitly configured cwd is honored verbatim — the Numbers tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Numbers in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Numbers install tree (%s) — set terminal.cwd to "
