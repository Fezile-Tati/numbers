"""OpenAI-shape bridge shared by Numbers' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Numbers as
...
the agent's *own* tool notifications. Numbers' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Numbers' perspective).
...
of its own (Copilot) forwards everything Numbers offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Numbers'
agent-level tools, because re-offering the overlapping ones makes Numbers re-run
...
class StreamChunks(list):
    ...
    Numbers reads provider-level extras off the object returned by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Numbers'
