...
def _ensure_cron_dir(cron_dir: Path) -> None:
    ...
    Default and custom Numbers homes keep ``parents=True`` so first-run
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Numbers timezone, not the server's local
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Numbers configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Numbers timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Numbers/system local time
...
def create_job(
    ...
    Args:
        ...
        reasoning_effort: Optional per-job reasoning effort pin. One of the
                canonical Numbers levels (none|minimal|low|medium|high|xhigh|
