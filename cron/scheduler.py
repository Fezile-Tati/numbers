...
# Without this, standalone invocations (e.g. after `numbers update` reloads
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Numbers, pin it explicitly: "
    ...
    # checkout was updated underneath it (interrupted `numbers update`, manual
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``numbers tools`` config for the ``cron`` platform.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``numbers update`` / ``hermes gateway stop`` / systemd restart, or an
...
def _get_hermes_home() -> Path:
    """Resolve Numbers home dynamically while preserving test monkeypatch hooks.
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Numbers — {task_name}"
    try:
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Numbers-managed secrets are not inherited
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `numbers setup`), or pin a "
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            ...
            return (
                ...
                "`numbers setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    try:
        ...
        #
        # A kanban worker is a normal `numbers chat -q` CLI agent whose default
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                "default with `numbers model <name>`."
            )
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Numbers "
                        "pin it explicitly: "
                        f"`hermes cron edit {job_id} --provider <provider> "
...
def tick(
    ...
    *,
    ...
    # ``git pull``, interrupted ``numbers update``) serves MIXED sys.modules:
