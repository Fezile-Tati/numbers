"""
Numbers MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Numbers-specific extra)
...
class EventBridge:
    ...
    This is the Numbers equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Numbers MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "hermes",
        instructions=(
            "Numbers messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Numbers MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
