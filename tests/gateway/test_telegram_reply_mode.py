...
class TestDMTopicFallbackReplyToMode:
    ...
    reply_to_mode 'off' was ignored when sending via Numbers-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    response in a different chat lane than the Numbers topic the session runs
    in. The no-anchor branch must prefer the Numbers topic's
