"""POSIX advisory locks must survive Numbers' own database inspection.
...
Numbers used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `numbers sessions
...
out across Numbers' inspection calls.
"""
