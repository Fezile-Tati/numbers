...
# the REAL Numbers root — capture it BEFORE the sandbox rewires HERMES_HOME,
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # any test run launched FROM a Numbers agent session inherits them and
...
def _capture_real_kanban_root() -> Path:
    ...
    2. the real (pre-sandbox) Numbers root otherwise
    """
...
# path that resolves inside the REAL Numbers root. This fixture wires the
# test-side knobs:
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Numbers home override redirects every later
...
@pytest.fixture()
def mock_config():
    """Return a minimal numbers config dict suitable for unit tests."""
    return {
...
def _wal_is_usable() -> bool:
    """True when Numbers will actually put a database into WAL mode here.

    Numbers refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Numbers can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Numbers managed runtime on 3.53.1
...
# OS gating
#
# Numbers runs on Linux, macOS and native Windows, and a lot of its behaviour
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Numbers falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Numbers uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # Block any subprocess that would run `numbers update` (or the
