...
Numbers's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Numbers root.  harden_import_path must relocate the
    Numbers root to the front so ``from utils import ...`` resolves to Numbers."""
    import sys
