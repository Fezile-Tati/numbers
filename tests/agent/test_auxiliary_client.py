...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``numbers auth status`` already reports these as logged in; compression
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``numbers model``, no per-task override is
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Numbers auth / credential pool."""
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Numbers owns the retry/timeout budget (issue #54465). The SDK
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Numbers reasoning.

    MoA slot reasoning is carried through call_llm as a Numbers
