...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    When the user picks "Anthropic API key" at `numbers setup`,
    ...
    + claude-cli user-agent).  Autodiscovered Claude Code / Numbers PKCE
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    `numbers setup` would still have those OAuth entries dormant on disk.
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    OAuth path at `numbers setup`.  When ANTHROPIC_TOKEN is set (and
...
def test_load_pool_skips_exchange_for_suppressed_copilot(tmp_path, monkeypatch):
    ...
    removed with ``numbers auth remove copilot gh_cli``.  The gate must run
