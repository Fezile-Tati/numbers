...
def wire(profile, monkeypatch):
    """Replace only HTTP transports; use Numbers routing and the real SDK."""
...
def test_new_identity_is_limited_to_the_official_endpoint(base_url, attributed):
    ...
    assert headers["User-Agent"] == (
        ...
        if attributed else "codex_cli_rs/0.0.0 (Numbers)"
    )
...
def test_credential_pool_custom_endpoint_keeps_existing_identity(
    ...
    try:
        ...
        assert wire[-1].headers["user-agent"] == "codex_cli_rs/0.0.0 (Numbers)"
