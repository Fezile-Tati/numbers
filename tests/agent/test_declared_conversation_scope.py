...
conversation-affinity hint Numbers sends — ``prompt_cache_key`` on both
...
bucket it warmed. Numbers cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-Numbers-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Numbers-internal token; a session KEY embeds the
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-Numbers-Session-Key`` accepts any authenticated caller-supplied string,
