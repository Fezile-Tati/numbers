...
class TestSafeRestore:
    """Safe restore: preserve user hand-edits, revert only Numbers-authored changes.
    ...
    def test_safe_restore_skips_file_user_edited_after_agent(self, mgr, work_dir):
        ...
        # Numbers writes the file, then the user modifies it afterwards.
        (work_dir / "main.py").write_text("agent version\n")
        mgr.record_agent_write(str(work_dir / "main.py"))
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # Content no longer matches what Numbers last wrote → preserved.
    ...
    def test_safe_restore_removes_agent_created_file_keeps_user_edit(self, mgr, work_dir):
        ...
        # Numbers creates a brand-new file after the checkpoint...
        ...
        result = mgr.restore(str(work_dir), base, safe=True)
        assert result["success"] is True
        # User edit preserved; Numbers-created file removed (not in checkpoint).
    ...
    # shape as a file Numbers created — and the delete branch treats absence as
