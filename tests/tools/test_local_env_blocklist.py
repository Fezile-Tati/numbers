...
Verifies that Numbers-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Numbers secrets.
...
class TestProviderEnvBlocklist:
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Numbers inference secret
        ...
        models`` run inside a Numbers terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        Numbers-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_bare_hermes_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Numbers even when the gateway PATH cannot."""
...
class TestActiveVenvMarkerStripping:
    ...
    into the Numbers venv path instead of the project's own ``.venv`` —
    silently clobbering the Numbers environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Numbers venv
...
class TestPythonpathSelectiveStrip:
    """PYTHONPATH Numbers-owned entry stripping (#74817).

    The Desktop Electron app injects the Numbers repo root and the Numbers
    ...
    entries Numbers itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Numbers-owned entries are removed; everything else survives
        ...
        cannot silently pass), duplicate Numbers entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Numbers ordering with an
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Numbers-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Numbers venv
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Numbers-owned entries.
        ...
        Numbers-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Numbers venv site-packages entries are stripped
        by the same Numbers-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local import _strip_hermes_owned_pythonpath
        import sys
        ...
        # the Numbers-owned check cannot match a backslash path here.
        assert hermes_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_hermes_owned_paths_stripped(self):
        """On Windows, a Numbers venv site-packages entry written with
        backslashes is stripped by the same Numbers-owned check, while a
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Numbers venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Numbers runtime producer contract.
        """
    ...
    def test_builders_strip_hermes_venv_pythonpath(self, builder):
        ...
        Numbers venv site-packages is stripped, user entries survive.
        """
    ...
    def test_scrub_child_env_strips_hermes_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Numbers venv
    ...
    def test_execute_code_composition_strips_inherited_hermes_entries(self, same_env):
        ...
        Seeds a contaminated inherited PYTHONPATH (Numbers repo root + Numbers
        ...
        * inherited Numbers venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        assert norm_venv not in norm_parts, \
            "inherited Numbers venv site-packages must be stripped"
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Numbers runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Numbers venv and crashes
...
class TestBlocklistCoverage:
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Numbers (#55878).
        """
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Numbers-managed
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Numbers, and blocklisting
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        (subscription OAuth), not a Numbers inference credential. Stripping it
...
class TestHermesInternalDynamicSecrets:
    """Dynamically-named Numbers secrets injected at gateway/CLI startup must
