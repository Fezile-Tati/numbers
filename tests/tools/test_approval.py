...
class TestHermesConfigWriteProtection:
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Numbers absolute config.yaml is
        # handled by the project patterns, not the Numbers-home rule.
        for cmd in (
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Numbers-home prefixes must fold to ~/ and
    ...
    SSH, and Numbers config/env files returned "safe" without an approval prompt.
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Numbers security config via IFS.
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Numbers service
    ...
    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Numbers labels, are
...
class TestLifecycleGuardLaunchctlParity:
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Numbers ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command
