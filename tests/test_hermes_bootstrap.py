...
The bootstrap module is imported at the top of every Numbers entry point
...
Key invariants covered by these tests:
  ...
  5. Load order: every Numbers entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Numbers CLI
...
class TestEntryPointsImportBootstrap:
    """Every Numbers entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke Numbers as a process.  Each one must
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``numbers update`` (git-reset landed new code but
...
class TestHardenImportPath:
    ...
    directory from shadowing Numbers's own top-level modules — covering both
