...
(``hermes-agent\\bin``) was swept by ``numbers update``'s autostash
...
existing install's PATH to the canonical layout from the ``numbers update``
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `numbers update` tail migration
# ---------------------------------------------------------------------------
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``numbers update`` autostashes with
    ...
    assert result.returncode == 0, (
        "bin/hermes.exe is not gitignored — numbers update's autostash "
