"""Tests for IRC gateway configuration via `numbers setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Numbers install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Numbers install."""
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `numbers setup gateway` experience with IRC present."""
