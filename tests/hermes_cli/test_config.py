...
class TestEnsureHermesHome:
    ...
    _PRE_REWRITE_DEFAULT_SOUL = (
        "You are Numbers, an intelligent AI assistant created by Nous "
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        ...
        Offering it as an editable env var (dashboard, `numbers setup`) lets a
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `numbers setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `numbers setup` to regenerate" in captured.out
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Numbers runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Numbers
...
class TestConfigCommandFailClosedSurface:
    """`numbers config set/unset` must exit cleanly (no traceback) when the
