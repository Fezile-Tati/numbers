...
class TestEnsureUvUpdateBoundary:
    ...
    ``numbers update`` runs the call site from the old, already-imported
...
class TestMinorLineFallForward:
    ...
    `numbers update` with no path to a fixed runtime.
    """
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every numbers update and stayed on journal_mode=DELETE
...
class TestVenvPythonUpdateBoundary:
    ...
    ``numbers update`` imports hermes_constants from the OLD checkout, ``git
...
class TestWindowsRuntimeSelfLock:
    ...
    A CLI ``numbers update`` runs from the venv's own python, and
