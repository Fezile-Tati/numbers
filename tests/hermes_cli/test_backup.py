...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Numbers backup."""
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Numbers configuration has been restored." in out
...
# ---------------------------------------------------------------------------
# Pre-update backup (numbers update safety net)
# ---------------------------------------------------------------------------
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``numbers update``
    ...
    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, hermes_home):
        """``numbers update`` in ``full`` mode takes the quick snapshot *before*
...
class TestPreMigrationBackup:
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Numbers
...
class TestRestoreCronJobsIfEmptied:
    """`numbers update` config migration can leave cron/jobs.json valid-but-empty,
