...
bare ``hermes`` and ``numbers chat`` (including the fast-chat launch path)
all go through it.
"""
...
def test_emit_hint_for_missing_split_model_config_default():
    ...
    buf = io.StringIO()
    assert emit_partial_update_hint(exc, file=buf) is True
    assert "numbers update" in buf.getvalue()
