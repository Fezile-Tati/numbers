"""Tests for the auxiliary-model configuration UI in ``numbers model``.
...
def test_delegation_cfg_as_task_projection():
    ...
    shaped = _delegation_cfg_as_task(
        {"delegation": {"provider": "nous", "model": "Numbers-4.5"}}
    )
    assert _format_aux_current(shaped) == "nous · Numbers-4.5"
