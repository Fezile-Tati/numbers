...
* Be supported-platform-only — no-op silently elsewhere so ``numbers update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Numbers
...
class TestRequireConfirmedUpdate:
    """`numbers update` passes require_confirmed_update=True: the full
    ...
    def test_incompatible_driver_repairs_on_posix_despite_indeterminate_check(self):
        """Numbers' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `numbers update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `numbers update`.
    """
...
class TestInstallerTimeoutDrainIsBounded:
    ...
    def test_drain_that_times_out_still_surfaces_the_run_timeout(self):
        ...
        so the existing manual re-run hint prints and ``numbers update``
