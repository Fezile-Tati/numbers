"""Tests for the Numbers plugin system (hermes_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    Pass *home* explicitly whenever the target Numbers home for this
...
class TestPluginDiscovery:
    ...
    def test_removed_relay_plugin_identity_cannot_be_reloaded(
        ...
        assert "Relay lifecycle is owned by Numbers core" in state.error
        assert RELAY_PLUGINS_CONFIG_ENV in state.error
        assert "Refusing to load removed Numbers Relay plugin" in caplog.text
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `numbers plugins list`. Attribution now counts what each plugin's own
