...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        The endpoint must mirror `numbers auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `numbers auth add`, which clears every suppression for the
...
class TestUpdateCheckEndpoint:
    ...
    ``POST /api/hermes/update`` runs ``numbers update``.
    """
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `numbers tools` configurator."""
