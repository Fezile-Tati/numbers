...
# load_pool() or a separate `numbers auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(hermes_home):
    ...
    the old cached credential until a separate ``numbers auth add opencode-go``
    ...
    authenticates immediately, matching ``numbers auth add <provider> --type
    api-key`` behavior.
    """
