...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `numbers model` overwrote it to `web.backend: nous`.
    """
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `numbers tools`
# ---------------------------------------------------------------------------
...
def test_has_agent_browser_import_failure_falls_back_to_hermes_managed_node_path(
    ...
    still find a runnable agent-browser under the Numbers Node dir even when
