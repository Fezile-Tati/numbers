...
dashboard-spawned ``numbers update`` was mid-``npm install``, which then failed
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``numbers update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Numbers is still running" and retrying
