"""Tests for the post-update *import* guard in ``numbers update``.
...
def test_hint_fires_for_first_party_import_error():
    ...
    assert any("numbers update" in line for line in hint)
...
def test_import_guard_prefers_the_project_venv_interpreter(monkeypatch, tmp_path):
    """``numbers update`` can run under a different Python than the install's.
