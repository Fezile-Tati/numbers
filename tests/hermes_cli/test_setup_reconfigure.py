...
- Bare `numbers setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `numbers setup --quick` runs the narrower "fill in missing items" flow.
- `numbers setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `numbers setup` on an existing install = full reconfigure wizard."""
