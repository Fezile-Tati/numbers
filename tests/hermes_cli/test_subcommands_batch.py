...
def test_config_get_unset_subcommands_parse():
    """`numbers config get/unset` parse key args (and --json for get)."""
...
# `hermes_cli/auth.py`) prints a deprecation notice pointing at `numbers auth` /
# `numbers model` and exits 0.  Two behavior contracts guard the UX:
