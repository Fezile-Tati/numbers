...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``numbers update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    # Exact flags used by numbers update (hermes_cli/main.py).
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    assert "made the Numbers agent unexecutable" in output
