...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool._find_agent_browser (PATH -> Homebrew/Numbers-managed
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """Numbers-managed-Node-only setups: npx resolves via
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `numbers tools` → "reconfigure existing" shows
...
#    reported as added/removed by `numbers tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `numbers tools` printed ``- kanban``
...
# `numbers tools post-setup <key>` child. On Windows each console child (npm,
...
# ── Toolsets that shipped after a platform's last `numbers tools` save ────────
#
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `numbers config set agent.disabled_toolsets '["memory"]'`). It must be
