...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `numbers auth add nous` must also populate providers.nous so the
    ...
    # raises "Numbers is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`numbers auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``numbers auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``numbers auth list`` showed two labels sharing
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """numbers auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``numbers auth add xai-oauth`` runs must produce independent pool entries.
...
# =============================================================================
# Unified credential-source stickiness — every source Numbers reads from has a
