...
other subprocess-influencing or Numbers-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next Numbers process would load it through the
...
def test_explicit_hermes_home_writes_to_that_env_file(tmp_path):
    ...
    Numbers home; keep that target while still routing through
