...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Numbers' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Numbers' DB/metadata/logs on task completion.
    """
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "Numbers" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
