...
"""
    ...
    assert "Tyler Williams > Active Projects: Numbers" in entries
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Numbers``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Numbers prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Numbers to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Numbers config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Numbers uses tools well"
    # All-lowercase matches → lowercase ``hermes``; this preserves the
    # real filesystem path ``~/.hermes`` (Numbers home) when rebranding
