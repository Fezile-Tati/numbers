...
2. `numbers update --check` works (update path)
...
def _run_hermes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    ...
    connectivity — but ``numbers update --check`` really does ``git fetch``
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `numbers update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`numbers update --check` must not load cryptography._rust."""
