...
left behind by a crashed Numbers.
"""
    ...
    if not binary:
        if os.name == "nt":
            return None, (
                ...
                "build. Set browser.engine to auto (or run Numbers under WSL2)."
            )
        return None, (
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Numbers is gone.

    Records are written by :func:`launch_lightpanda`; a live owner (another
    Numbers process, or this one still tracking the session) is never
