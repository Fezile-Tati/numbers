...
orchestrator work. A normal ``numbers chat`` session still sees **zero**
...
(``numbers dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``numbers chat`` without the kanban toolset see zero
...
def _connect(board: Optional[str] = None):
    ...
    the env-pinned active board without restarting Numbers.
    """
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; Numbers downloads it (with the shared size cap)
...
KANBAN_ATTACH_URL_SCHEMA = {
    ...
    "description": (
        "Attach a file to a task by URL — Numbers downloads it server-side "
