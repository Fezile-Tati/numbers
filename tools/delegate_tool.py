...
def _build_child_agent(
    ...
    # child Numbers/Qwen subagent onto the parent's Claude Messages wire (or the
...
def delegate_task(
    ...
    # request-scoped chat_id binding (the raw X-Numbers-Session-Id on
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Numbers-Session-Id once the batch completes.
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'numbers auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Numbers config.
    ...
    ``HERMES_IGNORE_USER_CONFIG=1`` (``numbers chat --ignore-user-config``) is
