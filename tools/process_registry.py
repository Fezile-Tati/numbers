...
# ---------------------------------------------------------------------------
# When Numbers runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Numbers gateway runtime.
...
class ProcessRegistry:
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `numbers update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Numbers desktop app.",
            }
...
def _delegation_model_not_found_notice(results) -> "list[str] | None":
    ...
    lines = [
        ...
        "numbers config get delegation.model).",
    ]
    try:
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Numbers'}"
    elif _reason == "lost":
