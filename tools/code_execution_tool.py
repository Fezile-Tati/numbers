...
Lets the LLM write a Python script that calls Numbers tools via RPC,
...
# available on every platform Numbers itself runs on.
...
# operational marker: without it, a sandbox script that spawns/imports Numbers
...
"""Auto-generated Numbers tools RPC stubs."""
...
"""Auto-generated Numbers tools RPC stubs (file-based transport)."""
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    # HERMES_TIMEZONE is an internal Numbers setting and must not leak
    ...
    # Numbers's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Numbers's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Numbers-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Numbers-
    ...
    if _uses_hermes_python_environment(child_python):
        ...
        logger.info(
            "execute_code: child interpreter %s is outside the Numbers "
...
# Interpreter paths already reported as outside the Numbers environment —
...
def _uses_hermes_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Numbers's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``numbers tools`` (e.g. web is turned off),
    ...
    if mode == "strict":
        ...
        cwd_note = (
            ...
            "Numbers's own python (the common case — stdlib plus Numbers's "
    ...
    description = (
        "Run Python that calls Numbers tools programmatically. Use when you "
