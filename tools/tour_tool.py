#!/usr/bin/env python3
"""Run a guided tour (highlight + narrate UI elements) in the Numbers desktop GUI.
...
- ``surface="app"`` — the Numbers desktop app's own DOM (tours of Numbers itself).
...
call fail would leave Numbers offering walkthroughs it cannot give.
"""
...
def tour_tool(
    ...
    if callback is None:
        return tool_error("tour is only available in the Numbers desktop app.")
...
TOUR_SCHEMA = {
    ...
    "description": (
        ...
        "element, attach a titled popover. Surfaces: 'app' (Numbers itself) "
