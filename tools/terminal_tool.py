...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Numbers exit
...
# display_hermes_home imported lazily at call site (stale-module safety during numbers update)
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # interactive Numbers password prompt or the sudo -S password-pipe path.
    ...
    # silently without Numbers prompting.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    by Numbers processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Numbers processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Numbers processes a generous grace
...
def _docker_persistent_profile_scoped() -> bool:
    ...
    ``container_persistent: true`` is ONE long-lived container per Numbers
...
def _current_session_profile() -> str:
    """Return the active session's Numbers profile name, or "" when unset.
...
def _ensure_terminal_env_bridged() -> None:
    ...
    be stale from ``numbers setup``). Environment values for omitted terminal
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Numbers processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Numbers tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        effective_task_id = _resolve_container_task_id(task_id)
        if _host_local:
            # Numbers-owned control-plane children must run beside the current
        ...
        # process that merely imports gateway.run (numbers serve --isolated,
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Numbers cron for one-shot delayed work, or install an "
