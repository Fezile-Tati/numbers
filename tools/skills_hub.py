#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Numbers Skills Hub.
...
class OptionalSkillSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's Numbers-native bundle
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``numbers update``. Rather than telling them to update first, resolve
...
# ---------------------------------------------------------------------------
# Numbers centralized index source
# ---------------------------------------------------------------------------
...
def _load_hermes_index() -> Optional[dict]:
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Numbers index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Numbers index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Numbers index fetch failed: %s", e)
            return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Numbers Skills Index.
