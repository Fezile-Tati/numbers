...
def sdk_httpx():
    ...
    side by side with Numbers' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Numbers' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Numbers explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Numbers and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Numbers Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Numbers MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Numbers gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Numbers document cache; returns a marker with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Numbers profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Numbers' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Numbers' logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `numbers setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Numbers process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Numbers' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Numbers is fully
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Numbers' pinned ``httpx``, and the two define unrelated
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Numbers processes (e.g.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Numbers config file.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Numbers' MEDIA: tag convention and out to messaging
            ...
            # Numbers' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Numbers's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Numbers uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Numbers registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``numbers tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Numbers registry.
