"""Base class for all Numbers execution environment backends.
...
# set), not Numbers' per-turn session identity.
#
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Numbers backends.
    ...
    # True only when commands execute on the SAME host as the Numbers process
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # under Numbers via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Numbers-specific
        ...
        # inherited from the Numbers process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Numbers running inside another agent's terminal).
        parts.append(
        ...
        # Restrict Numbers metadata files without changing the user's command
