...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Numbers was already launched from (for example ``hermes-agent``
    ...
    # Common recovery for config values like ``hermes-agent`` when Numbers was
...
# Numbers-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# Numbers-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Numbers inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Numbers-managed inference credential — Claude subscription auth is not a
    # working Numbers provider path.  Stripping it broke agent-spawned
...
# from the blocklist entirely because it is NOT a Numbers credential; these ARE
# Numbers-managed first-party platform credentials, so they stay IN the
...
# project's dependencies into the Numbers venv path instead of the project's own
# ``.venv`` — silently clobbering the Numbers environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Numbers venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Numbers venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Numbers itself treats PYTHONHOME as contamination in its own
...
# _strip_hermes_owned_pythonpath() which removes only Numbers-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_hermes_internal_secret(key: str) -> bool:
    """Return True for Numbers-internal secrets injected under *dynamic* names.
    ...
    This is the single source of truth for "Numbers-internal dynamic secret"
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local Numbers home override into subprocess env."""
    try:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Numbers-managed secrets from a subprocess environment."""
    try:
    ...
    # the separate Numbers runtime venv.  The filter validates that relationship
...
# legitimate child Numbers spawns needs them, and they are the highest-value
...
def hermes_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Numbers spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HERMES_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _plugin_terminal_env_strip_keys():
        env.pop(key, None)
    # Internal routing hints and Numbers-internal dynamic secrets
...
def build_subprocess_env(
    ...
    *,
    ...
    Numbers secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Numbers home override into ``HERMES_HOME`` and apply the
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Numbers requires Git for Windows on Windows.\n"
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Numbers. If the override is blocked or later re-applied, "
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Numbers-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Numbers installs for
    itself, so on a machine where Numbers provisioned its own toolchain a
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Numbers-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Numbers
...
def _build_hermes_repo_root_aliases(
    ...
    """Return exact repo-root spellings emitted by Numbers launchers.
    ...
    without treating arbitrary descendants of HERMES_HOME as Numbers-owned.
    ...
    # strict resolve before accepting it as Numbers-owned.  Fail-closed: a
...
# --- Numbers venv / repo-root detection (module-level, computed once) ---

#: The Numbers repository root - three levels up from this file
...
#: etc.  Subprocesses that are NOT the Numbers backend don't need it and it
#: can shadow local packages.
_hermes_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Numbers launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Numbers-owned paths under
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Numbers repo>/venv`` layout and a real venv marker; require both before
...
def _get_hermes_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Numbers runtime.
...
def _strip_hermes_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Numbers-owned PYTHONPATH entries, then the runtime marker vars.
...
def _strip_hermes_owned_pythonpath(env: dict) -> None:
    """Remove Numbers-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Numbers repo root and the Numbers venv's
    ...
    user entries, so only entries proven Numbers-owned are removed:
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Numbers-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Numbers venv site-packages ---
    ...
    if stripped:
        logger.debug(
            "Stripped Numbers-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Numbers trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Numbers host itself — controller-side platform
    ...
    def get_temp_dir(self) -> str:
        ...
        **Default (no override set):** a dedicated cache dir under
        ...
        tmpfs, and Numbers session artifacts — background-process logs,
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        # Honored ahead of the generic TMPDIR so users can redirect Numbers' temp
        ...
        # because RAM-backed tmpfs /tmp fills up under Numbers load.
        try:
            from hermes_constants import get_hermes_home
            cache_dir = get_hermes_home() / "cache" / "terminal"
            cache_dir.mkdir(parents=True, exist_ok=True)
