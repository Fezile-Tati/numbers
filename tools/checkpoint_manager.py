...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Numbers convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Numbers wrote
...
def _git_env(
    ...
    The shared store is internal Numbers infrastructure — it must NOT inherit
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.name", "Numbers Checkpoint"], store, cfg_wd)
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Numbers just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Numbers last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Numbers last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Numbers'
        last write or that Numbers never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Numbers has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Numbers never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Numbers wrote it: restoring it back is
                # safe — its last content was Numbers-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Numbers changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Numbers changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Numbers' last write — per the agent-write ledger —
        are left untouched, and only Numbers-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Numbers-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # Numbers created it. Deleting it would not restore a prior
