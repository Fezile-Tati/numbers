...
# means a compromised transitive dependency could read every Numbers secret
...
def _build_browser_env() -> dict:
    ...
    Strips Numbers-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Numbers profile."""
...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    if bu_mode:
        return True, "Browser Use mode: Numbers spawns `lightpanda serve` per session"
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Numbers silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Numbers-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Numbers CLI where sys.stdout and sys.stderr both report
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # ``browser.engine: lightpanda`` that endpoint is a Numbers-spawned
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Numbers-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Numbers-managed one with no recovery — every candidate
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Numbers-managed
    ...
    # node_modules binary, which disappears on the next ``numbers update`` and
    ...
    # Build an extended search PATH including Numbers-managed Node, macOS
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``numbers update`` (or ``hermes doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Numbers holds, while running registry-fetched
    npm code on every ``numbers update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `numbers update`/`doctor
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Numbers
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if session_info:
        ...
        # A Lightpanda session is a process Numbers spawned itself (Browser
