...
# Numbers session id (observability identity, distinct from the gateway
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Numbers session id so observer plugins parent approval
...
def _is_single_query_approval_context() -> bool:
    ...
    ``numbers chat -q "..."`` runs one turn and exits with no user waiting to
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Numbers: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Numbers
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]hermes[^\n]*\.env\b', "access to Numbers secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    (r'\bhermes\s+update\b', "numbers update (restarts gateway, kills running agents)"),
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Numbers file.
    ...
    # In-place edit of a Numbers-managed security file (~/.hermes/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Numbers config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Numbers config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Numbers config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    #
    # Fold the (more specific) Numbers home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hermes), so folding the user home
    # first would eat the prefix the Numbers-home fold needs.
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    """Rewrite the resolved absolute Numbers home prefix to ``~/.hermes/``.
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Numbers ad-hoc temp script."""
    try:
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Numbers YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Numbers approval prompts.
