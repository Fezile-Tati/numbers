...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Numbers.
...
def _get_hermes_config_resolved() -> str | None:
    """Return the resolved absolute path of the Numbers config file (cached)."""
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Numbers config file directly.
    ...
    if hermes_config and (resolved == hermes_config or normalized == hermes_config):
        return (
            f"Refusing to write to Numbers config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.hermes/config.yaml directly or use 'numbers config' instead."
        )
    return None
...
# .cursorrules (or a project-local .numbers config tree) outlives the current
...
def _get_real_hermes_home() -> str | None:
    """Return the realpath of the authoritative Numbers home (cached)."""
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .numbers config dirs (e.g. <repo>/.hermes/config.yaml)
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Numbers mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Numbers state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Numbers process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside Numbers scope.
    """
    try:
