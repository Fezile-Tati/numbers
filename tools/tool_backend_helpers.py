...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `numbers model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``numbers auth add
    ...
    3. The credential pool / auth store for ``provider_id`` (``numbers auth
    ...
    try:
        ...
        # `numbers auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``numbers auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `numbers tools` provider string for a config section.
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        ...
        f"tools), but {failure}. Run 'numbers tools' to change it."
    )
