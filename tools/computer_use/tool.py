...
# regardless of approval level (e.g. logout kills the session Numbers runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Numbers-session cached backends. Each backend owns its own cua-driver
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Numbers's explicit approval bypass onto Cua's immutable mode.

    Numbers has TWO session-identity namespaces: the tool-dispatch path passes
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Numbers YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Numbers sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Numbers process that spawned it
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in Numbers' media cache and return its absolute path.
