...
flow through here without code changes on the Numbers side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Numbers provider secrets stripped.
...
def _build_identity(binary: str, report: Dict[str, Any]) -> Dict[str, Any]:
    """Numbers-side identity block comparing resolved binary vs health_report."""
...
def run_doctor(
    ...
    *,
    ...
    if json_output:
        ...
        # attach Numbers identity under hermes_identity so existing parsers
