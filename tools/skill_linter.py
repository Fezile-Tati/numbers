...
Design contract (matches the Numbers "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    if author and author.strip().lower() in ("hermes", "agent", "numbers agent") and (
        author != "Numbers"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Numbers' (proper caps) "
