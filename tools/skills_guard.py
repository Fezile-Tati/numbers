...
Trust levels:
  - builtin:   Ships with Numbers. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.hermes/\.env|\~/\.hermes/\.env',
     ...
     "directly references Numbers secrets file"),
    ...
    #     For Numbers/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_HERMES_CONFIG_FILES),
     ...
     "modification language aimed at Numbers configuration files (verify intent)"),
    (_shell_write_re(_HERMES_CONFIG_FILES),
     "hermes_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting Numbers configuration files"),
    (r'\.hermes/config\.yaml|\.hermes/SOUL\.md',
     "hermes_config_ref", "low", "persistence",
     "references Numbers configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the Numbers-native name; `.clawhubignore` is honored
