"""ACP auth helpers — detect and advertise Numbers authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Numbers runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Numbers can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Numbers.
    ...
    not have Numbers provider credentials configured yet, so Numbers always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Numbers using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Numbers provider",
            description=(
                "Open Numbers' interactive model/provider setup in a terminal. "
                "Use this when Numbers has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
