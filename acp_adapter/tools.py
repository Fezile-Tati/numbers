"""ACP tool-call helpers for mapping numbers tools to ACP ToolKind and building content."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Numbers tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Numbers tool result clearly failed.
    ...
    # Numbers core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # Numbers read_file output is line-numbered with `|`. If we send it as raw
