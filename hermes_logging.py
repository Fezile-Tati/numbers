"""Centralized logging setup for Numbers.
...
# essentially always in Numbers (TUI, gateway, ``hy_memory`` server, MCP
...
def setup_logging(
    *,
    ...
    """Configure the Numbers logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Numbers home directory.  Falls back to
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Numbers home.
...
# the module header): when several Numbers processes log to the same file, an
