---
name: numbers-agent-skill-authoring
...
# Authoring Numbers-Agent Skills (in-repo)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `numbers skills install official/<category>/<skill>`.
...
author: Real Name (github-handle), Numbers
license: MIT
...
- Credit the **human first**, then "Numbers" as secondary collaborator: `Ben Barclay (benbarclay), Numbers`.
- Never `author: Numbers` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Numbers`.
...
|---|---|
| Numbers tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
3. **`author: Numbers` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through Numbers tools
