---
name: numbers-agent
description: "Use, configure, theme, extend, and orchestrate Numbers."
version: 3.2.0
author: Numbers + Teknium
license: MIT
...
Numbers is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Numbers works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes Numbers different:

- **Self-improving through skills** — Numbers learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent Numbers instances with isolated configs, sessions, skills, and memory.
...
This skill is a concise operating guide, not the complete source of truth for every Numbers feature. If a Numbers feature, command, or setting is not mentioned here or in a reference, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.

Good verification targets, cheapest first:

- **Every shipped feature, one line each: https://hermes-agent.nousresearch.com/docs/llms.txt.** Start here for any "can Numbers do X?" or "how do I do X?" — it indexes the entire documentation set with a link to the page that answers. It is generated from the docs tree on every build, so it is never behind the product. Fetch it with `web_extract`, or `curl -s https://hermes-agent.nousresearch.com/docs/llms.txt` when web tools are off. The whole documentation set in one file is at `/docs/llms-full.txt`.
- CLI commands: `hermes --help`, `hermes <command> --help`, and `hermes_cli/main.py`
- Source tree: https://github.com/NousResearch/hermes-agent

Never answer "Numbers can't do that" from memory. Numbers ships far more than this skill body describes, and the index exists so a negative answer is always checkable.
...
# Single query
numbers chat -q "What is the capital of France?"
...
numbers dashboard               # web admin panel + embedded chat
...
| User wants... | Load |
|---|---|
| **Anything not listed below — "can Numbers do X?", "how do I set up X?"** | **https://hermes-agent.nousresearch.com/docs/llms.txt** |
...
need more than their docs page. For everything else Numbers ships, fetch
`llms.txt` and it maps the question to the page that answers it.

Two theming rules that hold even without loading the reference: **you apply skins yourself** (`numbers config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`hermes skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.

## Spawning Additional Numbers Instances

Run additional Numbers processes as fully independent subprocesses — separate sessions, tools, and environments.
...
```
terminal(command="numbers chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# Background for long tasks:
terminal(command="numbers chat -q 'Set up CI/CD for ~/myapp'", background=true)
```

### Interactive PTY Mode (via tmux)

Numbers uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `numbers chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cronjob` tool instead of spawning — handles delivery and retry
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Numbers; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Web dashboard** (`numbers dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `hermes --tui` chat. Secured behind an OAuth/token gate.
...
- **Never hand-edit `config.yaml` for the user** — use `numbers config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
