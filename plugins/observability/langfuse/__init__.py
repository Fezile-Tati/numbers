"""langfuse — Numbers plugin for Langfuse observability.

Traces Numbers conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Numbers plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``numbers plugins enable
observability/langfuse`` or ``numbers tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``numbers tools`` or ~/.hermes/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Numbers plugin system —
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `numbers tools` and configure Langfuse "
        ...
        # short-lived-process gap (kanban workers / numbers chat -q / cron): exit
...
def _trace_key(
    ...
    *,
    ...
    Older Numbers paths only expose ``task_id``/``session_id``. Newer paths
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical Numbers usage into Langfuse usage and cost maps."""
    ...
    # Numbers estimate rather than summing components because it also includes
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``numbers chat -q`` one-shots, cron jobs — can exit while
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Numbers branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Numbers also has a turn-scoped
    ...
    # messages is a list only for legacy Numbers branches that fired
    # pre_llm_call with API messages directly. Current Numbers fires
...
def register(ctx) -> None:
    ...
    # Numbers versions.  pre_api_request / post_api_request fire per API
