"""
Photon Spectrum (iMessage) platform adapter for Numbers.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Numbers dep
...
# artifacts so Numbers sees the link once, not a follow-up "(attachment)" prompt.
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # `numbers setup` / status surface the missing-deps state.
...
def _sidecar_deps_stale() -> bool:
    ...
    `numbers update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Numbers does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Numbers defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Numbers-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Numbers checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        ...
        # A `numbers update` that bumps the spectrum-ts pin rewrites
...
async def _standalone_send(
    ...
    *,
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Numbers gateway (which spawns the sidecar and "
    ...
    headers = {"X-Numbers-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Numbers plugin loader at startup."""
