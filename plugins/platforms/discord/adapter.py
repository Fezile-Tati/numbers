...
def _needs_server_members_intent(
    ...
    """Return True when Numbers must request Discord's Server Members intent.
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Numbers gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Numbers requested (#79430): Message
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Numbers response. Emoji-only acks are
    ...
    async def _should_backfill_discord_message(self, message: Any) -> bool:
        """Return True when a recent Discord message still needs Numbers work."""
    ...
    def _is_down_notice_content(self, content: str) -> bool:
        """Recognize only explicit Numbers/gateway outage notices."""
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Numbers manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Numbers pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Numbers session")
        ...
        @tree.command(name="status", description="Show Numbers session status")
        ...
        @tree.command(name="stop", description="Stop the running Numbers agent")
        ...
        @tree.command(name="update", description="Update Numbers to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Numbers gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Numbers session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Numbers in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Numbers skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Numbers session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Numbers: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Numbers has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Numbers"
        if len(content) > 80:
            ...
            try:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Numbers: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Numbers semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Numbers session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Numbers handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Numbers to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Numbers needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Numbers needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``numbers update --gateway``
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Numbers could not create a Discord thread for "
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``numbers update`` prompts.
...
def register(ctx) -> None:
    """Plugin entry point — called by the Numbers plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `numbers setup` to install Discord support.",
