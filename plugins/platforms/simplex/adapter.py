"""SimpleX Chat platform adapter (Numbers plugin).
...
This adapter ships as a Numbers platform plugin under
``plugins/platforms/simplex/``. The Numbers plugin loader scans the
...
discoverable and ``numbers setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Numbers — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``numbers setup gateway`` → SimpleX.
