...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Numbers media cache directory.
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Numbers-managed Node/npm so Windows installs are not broken by a
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # `numbers update` bumps the Baileys pin).  The stamp file records
            ...
            if not _deps_fresh:
                ...
                # Numbers-managed portable Node before falling back to PATH.
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `numbers update`, so without this check it
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``numbers setup`` even
