...
# identify HermesAgent traffic — matching other Numbers outbound surfaces
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``numbers config set`` and Railway persist YAML
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Numbers has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Numbers can "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Numbers handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Numbers session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Numbers owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message_impl(
        ...
        # the shape produced by peer Numbers agents in Socket Mode on some
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Numbers",
                bot_description="Your Numbers agent on Slack",
            )
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `numbers update`)",
                True,
            ):
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `numbers setup` to install Slack support.",
