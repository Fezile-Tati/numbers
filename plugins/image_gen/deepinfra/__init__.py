...
models show up in ``numbers tools`` automatically. No model ids are
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `numbers tools` → Image "
                    "Generation → DeepInfra to configure, or `numbers setup` "
                    "to add the key."
                ),
