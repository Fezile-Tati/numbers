...
three virtual model IDs so the ``numbers tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `numbers tools` → Image "
                    "Generation → OpenAI to configure, or `numbers setup` "
                    "to add the key."
                ),
