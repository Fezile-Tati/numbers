...
``numbers plugins remove`` deletes it and ``numbers plugins update`` git-pulls
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Numbers.
...
# Mirrors the plugin-name shape `numbers plugins install` accepts. Anything
