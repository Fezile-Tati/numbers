...
``{title, url, description, position}`` rows every other Numbers web
provider produces.
...
prefers Numbers-managed xAI Grok OAuth (via ``numbers auth``) and falls back
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``numbers tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `numbers auth` to sign in with "
