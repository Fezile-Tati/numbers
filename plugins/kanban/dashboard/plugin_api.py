...
This means ``numbers dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def _active_profile_name() -> str:
    """Return the current Numbers profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    dropdown can never offer a model/provider pair the rest of Numbers
...
def list_kanban_projects():
    """List first-class Numbers projects for board scoping.
