...
def cmd_sync(args) -> None:
    ...
    Scans all Numbers profiles and creates host blocks for any that don't
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `numbers update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Numbers profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Numbers persistent cross-session memory.")
    ...
    # These keys only affect the Numbers GATEWAY (Telegram/Discord/Slack/...),
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Numbers gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    try:
        ...
        print("  Run: numbers config set memory.provider honcho")
...
def _active_profile_name() -> str:
    """Return the active Numbers profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Numbers' identity in Honcho. Seed with 'hermes honcho identity <file>'.")
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Numbers asks Honcho's AI peer a question")
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Numbers + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Numbers\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Numbers persistent memory")
    ...
    if user_files:
        ...
        print("  (Numbers calls migrate_memory_files() on first session init.)")
        print()
    ...
    print()
    print("  In Numbers, they are seeded once into Honcho's AI peer through the")
    ...
    print("    Numbers:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Numbers:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Numbers:   Honcho observes every message and updates representations")
    ...
    print("    Numbers:   per-session by default — each run gets its own session")
...
def register_cli(subparser) -> None:
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Numbers Honcho",
    )
