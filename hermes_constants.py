...
def set_hermes_home_override(path: str | Path | None) -> Token:
    """Set a context-local Numbers home override and return its reset token.
...
def reset_hermes_home_override(token: Token) -> None:
    """Restore the previous context-local Numbers home override."""
    _HERMES_HOME_OVERRIDE.reset(token)


def get_hermes_home_override() -> str | None:
    """Return the active context-local Numbers home override, if any."""
...
def _get_platform_default_hermes_home() -> Path:
    """Return the platform-native default Numbers home path."""
...
def get_hermes_home() -> Path:
    """Return the Numbers home directory (default: platform-native path).
...
def hermes_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Numbers home/profile directory.
...
def get_process_hermes_home() -> Path:
    """Return the Numbers home for the running process, ignoring task overrides.
...
def get_default_hermes_root() -> Path:
    """Return the root Numbers directory for profile-level operations.

    In standard deployments this is the platform-native Numbers home
...
# Files whose presence marks a directory as a real Numbers home. A fresh home
...
def _is_hermes_profiles_root(profiles_dir: Path) -> bool:
    ...
    provably live under a Numbers home: the classic ``~/.hermes`` layout, a
    root carrying Numbers-home marker files (Docker/custom ``HERMES_HOME``
    like ``/opt/data``), a ``profiles/.deleted`` tombstone directory (only
    ever created by ``hermes profile delete``), or the process's resolved
    default Numbers root.
    """
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Numbers home (see :func:`_is_hermes_profiles_root`). A default Numbers home
    ...
    for candidate in (current, *current.parents):
        if (
            ...
            return candidate
        # Stop at a default Numbers home so a coincidental ``profiles/``
...
def get_hermes_dir(
    ...
    *,
    ...
    """Resolve a Numbers subdirectory with backward compatibility.
    ...
    Args:
        ...
        home: Optional explicit Numbers home. Profile-aware callers that manage
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Numbers-managed Node.js directories in preferred lookup order.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Numbers-managed Node trees live under ``$HERMES_HOME/node`` (or a profile's
    ...
    used to trust file presence alone, so ``numbers update`` would pick that
...
def hermes_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Numbers-managed node/npm/npx shim exists on disk."""
    names = set()
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Numbers-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `numbers update` afterwards).",
        flush=True,
    )
...
def bootstrap_hermes_managed_node() -> str | None:
    """Install a Numbers-managed Node tree and return its npm path.
    ...
    Numbers never modifies a toolchain it does not own, so instead it provisions
...
def heal_hermes_managed_node() -> bool:
    """Redownload Numbers-managed Node when the tree exists but is broken.
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a Numbers-managed Node/npm executable path, healing broken trees.
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Numbers-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Numbers-managed installs.

    This is for Numbers-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Numbers-managed Node directories prepended to PATH."""
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``numbers update`` — leaving a **dangling symlink** that ``which`` still
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Numbers profile HOME.

    ``HERMES_HOME`` scopes Numbers state. ``HOME`` is reserved for the OS/user
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Numbers' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Numbers itself runs inside WSL.
...
# Top-level packages/modules that ship as part of Numbers itself. An ImportError
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Numbers.
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `numbers update` is exactly
