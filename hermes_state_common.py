...
# optimized via ``numbers sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Numbers-Session-Key / build_session_key)
...
# `numbers sessions optimize-storage` rebuilds it on a capable host.
...
#
# Several independent Numbers processes routinely share one state.db (gateway
# service, the Desktop app's `numbers serve` backend, interactive CLI sessions,
