"""``hermes logs`` — view and filter Numbers log files.
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Numbers runs — try 'numbers chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'numbers chat' to generate logs)")
