#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Numbers Skills Hub.

Powers both:
  - `numbers skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # skill: `numbers skills install impeccable` should resolve to the
        ...
        # so users can copy it for `numbers skills install`.
...
def _resolve_source_meta_and_bundle(identifier: str, sources):
    ...
    skill is how ``numbers skills inspect owner/repo/skills/foo`` showed
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `numbers skills install` fail.
    ...
    c.print(table)
    c.print("[dim]Use: numbers skills inspect <identifier> to preview, "
            "numbers skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # The identifier is what you pass to `numbers skills install`. Browse used
    ...
    c.print("[dim]Tip: 'numbers skills inspect <identifier>' to preview, "
            "'numbers skills install <identifier>' to install, "
            "'numbers skills search <query>' to search deeper[/]\n")
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]numbers skills install {url} --name <your-name>[/]\n\n"
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="numbers skills install <id> to install"))
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    already get from ``numbers update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: numbers skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `numbers update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `numbers update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   numbers skills diff <name>[/]")
    c.print("[dim]Resume updates: numbers skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: numbers skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: numbers skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: numbers skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: numbers skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: numbers skills publish <path> --to github --repo owner/repo\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Numbers Skills Hub.\n\n"
                        f"This skill was scanned by the Numbers Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `numbers skills <subcommand>` — called from hermes_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: numbers skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: numbers skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: numbers skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'numbers skills <command> --help' for details.\n")
