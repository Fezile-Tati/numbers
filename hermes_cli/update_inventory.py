...
One read-only pass that answers, BEFORE any mutation: what Numbers runtimes
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Numbers runtime on this machine."""
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Numbers services: none detected — code swap only.")
        return
