...
Three layers that make every long-lived Numbers process positively
identifiable, so reapers (``numbers update``, Desktop startup sweeps) never
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Numbers root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Numbers install on the
...
def register_self(
    ...
    *,
    ...
    try:
        ...
        # 10 tokens (was 6): enough for `numbers serve --host X --port N
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import Numbers code). The entry records the
