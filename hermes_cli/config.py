"""
Configuration management for Numbers.
...
#   the next ``subprocess.run([...])`` Numbers makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Numbers itself starts
...
#   ``HERMES_ENV`` — Numbers runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Numbers runtime location — never via dashboard env writer.
    ...
    # Numbers security policy / approval-routing context. These remain available
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Numbers runtime location "
...
def is_managed() -> bool:
    """Check if Numbers is running in package-manager-managed mode.
...
_NIX_UPDATE_MSG = (
    "Update Numbers through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Numbers was installed: 'apt', 'docker', 'nix', 'nixos',
...
#     — that script installs a *new* host-side Numbers, it doesn't update
...
Numbers runs as a published image (nousresearch/hermes-agent), not a
...
def format_managed_message(action: str = "modify this Numbers installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Numbers installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Numbers."
    )
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    Docker containers running Numbers commonly set these to map the in-container
...
def _is_container() -> bool:
    ...
    When Numbers runs in a container with volume-mounted config files, forcing
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Numbers' own config writer has historically emitted it
    ...
    # Numbers (``_save_discovered_models_to_config``), not hand-curated.
    # Older Numbers versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Numbers versions) may write
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Numbers won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Numbers host rewrites it to the host or container
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Numbers writes itself."""
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Numbers — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Numbers vars that are no longer in .env
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    ...
    Used for Numbers-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Numbers Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
