"""hermes import-agent — import Claude Code / Codex CLI setups into Numbers.
...
deliberately via ``numbers setup`` or config.yaml.
"""
...
# Same entry delimiter as the Numbers memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`numbers config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Numbers command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Numbers glob.
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Numbers equivalent — "
...
def import_agent_command(args) -> None:
    ...
    print(color("│          ⚕ Numbers — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
    ...
    print_info("Secrets:     never imported — run 'numbers setup' for credentials")
    ...
    print_info("API keys and credentials were NOT imported — run 'numbers setup' "
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'numbers setup' or ~/.hermes/.env.")
        print()
