"""Per-provider model-selection wizard flows for ``numbers setup`` / ``numbers model``.
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # bad paste short of re-running `numbers setup` from scratch. OpenRouter
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `numbers auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``numbers auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `numbers model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    print("Anthropic-style API endpoints.  Numbers will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Numbers will install it now (the preflight below "
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Numbers turns to `copilot --acp`.")
    print("  Numbers currently starts its own ACP subprocess for each request.")
    print("  Numbers uses your selected model as a hint for the Copilot ACP session.")
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `numbers auth`)
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Numbers typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Numbers, enable billing on your "
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'numbers model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
