...
TIPS = [
    ...
    "Ctrl+Z suspends Numbers to the background — run fg in your shell to resume.",
    ...
    "numbers chat -t web,terminal enables only specific toolsets for a focused session.",
    "numbers chat -s github-pr-workflow preloads a skill at launch.",
    "numbers chat -q \"query\" runs a single non-interactive query and exits.",
    "numbers chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "numbers chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "numbers chat --source telegram tags the session for filtering in numbers sessions list.",
    ...
    "numbers config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "numbers config edit opens config.yaml in your default editor.",
    "numbers config check scans for missing or stale configuration options.",
    "numbers sessions browse opens an interactive session picker with search.",
    "numbers sessions stats shows session counts by platform and database size.",
    "numbers sessions prune --older-than 30 cleans up old sessions.",
    "numbers skills search react --source skills-sh searches the skills.sh public directory.",
    "numbers skills check scans installed hub skills for upstream updates.",
    "numbers skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "numbers skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hermes mcp add github --command npx adds MCP servers from the command line.",
    "hermes mcp serve runs Numbers itself as an MCP server for other agents.",
    "numbers auth add lets you add multiple API keys for credential pool rotation.",
    ...
    "hermes backup creates a zip backup of your entire Numbers home directory.",
    ...
    "numbers update syncs new bundled skills to ALL profiles automatically.",
    "hermes gateway install sets up Numbers as a system service (systemd/launchd).",
    ...
    "Save money: numbers tools disables unused tools, numbers skills config trims skills down.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Numbers tools programmatically — results stay out of context.",
    ...
    "When exiting, Numbers prints a resume command with session ID and stats.",
    "numbers sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "numbers skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Gateway & Messaging ---
    "Numbers runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    ...
    "BlueBubbles brings iMessage to Numbers via a local macOS server.",
    ...
    "Numbers loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — numbers tools can toggle them per platform.",
    ...
    "numbers chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "numbers plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hermes acp runs Numbers as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "numbers auth add nous or numbers auth add openai-codex sets up OAuth-based providers.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Numbers your own.",
    ...
    "Before a gateway session reset, Numbers auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Numbers state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via numbers tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Numbers auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Numbers config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Numbers is working.',
    ...
    'numbers auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'numbers dashboard launches a local web UI at 127.0.0.1:2149 — zero data leaves localhost.',
    'numbers dashboard embeds the full Numbers TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'numbers chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'numbers chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'numbers chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "numbers chat --source tool tags programmatic chats so they don't clutter numbers sessions list.",
    'hermes dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'numbers sessions rename <ID> "new title" renames any past session; numbers sessions delete <ID> removes one.',
    'hermes import restores a full Numbers backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    'numbers setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Numbers block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from numbers sessions list by default — set --source explicitly to see them.',
...
# any project or none — Numbers is not a coding-only agent.
# ---------------------------------------------------------------------------
