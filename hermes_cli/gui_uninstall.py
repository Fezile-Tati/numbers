"""
Numbers Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Numbers.app`` or ``~/Applications/Numbers.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Numbers`` (NSIS per-user)
...
the app name ("Numbers"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Numbers``
  - Windows: ``%APPDATA%\\Numbers``
  - Linux:   ``$XDG_CONFIG_HOME/Numbers`` (default ``~/.config/Numbers``)
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Numbers"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Numbers"
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Numbers" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            ...
            home / "Applications" / "Numbers.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Numbers).
            local_base / "Programs" / "Numbers",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Numbers")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Numbers.desktop",
...
def uninstall_gui(
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Numbers disappears from the launcher.
        try:
