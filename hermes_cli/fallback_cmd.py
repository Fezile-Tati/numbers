...
Subcommands:
  ...
  hermes fallback add      Pick provider + model via the same picker as `numbers model`,
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `numbers model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `numbers model`, then append the selection to the chain."""
    ...
    print("  `numbers model` — select the provider + model you want as a fallback.")
    print()
