...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → Numbers + Honcho
    hermes --version           Show version and update status
    numbers update              Update to latest version
    hermes uninstall           Uninstall Numbers
...
def _apply_profile_override() -> None:
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        ...
        ``--profile``), not to Numbers' own profile selector.
        """
        try:
...
# Numbers root's ``bin``, next to the managed uv) by install.ps1. That dir
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Numbers itself has been explicitly configured (model
    ...
    # Only count these if Numbers has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Numbers to use their tokens.
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    print(
        "Error: the TUI workspace is missing from this Numbers checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `numbers update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Numbers checkout, run `git restore -- ui-tui`\n"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        # and exit on an install whose only Node is the one Numbers installed,
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            # here before doing any work; repair once (upgrade a Numbers-managed
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Numbers — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Numbers still functions without them; the user just sees an
...
def cmd_chat(args):
    ...
    # Code / Codex CLI) and resume the newly created Numbers session.
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Numbers isn't configured yet -- no API keys or providers found."
        )
        print()
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Numbers?")
        print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Numbers'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
#
# Numbers uses lightweight "auxiliary" models for side tasks (vision analysis,
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Numbers only falls back to a lightweight backend (OpenRouter,")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Numbers URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                f"    Numbers will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Numbers CLI with a provider."""
...
def cmd_backup(args):
    """Back up Numbers home directory to a zip file."""
...
def cmd_import(args):
    """Restore a Numbers backup from a zip file."""
...
def cmd_uninstall(args):
    """Uninstall Numbers (or just the Chat GUI with --gui)."""
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    # of a Numbers-managed install, or a freshly provisioned managed npm when
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    def _install_web_deps(*, silent: bool) -> "subprocess.CompletedProcess":
        return _run_npm_install_deterministic(
            npm,
            npm_cwd,
            # NUMBERS deviation: `.npmrc` sets engine-strict=true and the repo
            # pins a high Node floor (^22.22.0); on an older-but-workable Node
            # (e.g. 22.17) the install would hard-fail EBADENGINE and the build
            # would never run. Downgrade the engine check to a warning so the
            # web UI still builds. (Upgrading Node past the floor is the real
            # fix and also lets stock `hermes dashboard` build.)
            extra_args=(*npm_workspace_args, "--engine-strict=false", "--silent", "--prefer-offline") if silent else (*npm_workspace_args, "--engine-strict=false", "--prefer-offline"),
            env=build_env,
        )
...
#   - ``numbers update`` can unconditionally call ``hermes desktop --build-only``
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Numbers.app/Contents/MacOS/Numbers → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Numbers.app/Contents/MacOS/Numbers"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Numbers.exe",
            release_dir / "win-ia32-unpacked" / "Numbers.exe",
            release_dir / "win-arm64-unpacked" / "Numbers.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "hermes",
            release_dir / "linux-unpacked" / "Numbers",
            release_dir / "linux-arm64-unpacked" / "hermes",
            release_dir / "linux-arm64-unpacked" / "Numbers",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Numbers.exe
...
# `numbers update` → `hermes desktop --build-only` → relaunch) rebuilds
# Numbers.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Numbers.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Numbers.exe``.
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Numbers.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Numbers.exe from backup.")
    ...
    print("    Run `hermes desktop --force-build` to rebuild, or re-run the Numbers")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Numbers'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Numbers.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Numbers.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Numbers.exe: Access is
    ...
    an unrelated "Numbers" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Numbers.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Numbers is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Numbers.app/Contents/MacOS/Numbers  ->  app bundle = .../Numbers.app
    app = exe.parents[2]
...
def _desktop_macos_setup_tcc_identity(identity: str = "Numbers Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Numbers to use it.
    ...
    # Point Numbers at the identity (config.yaml, not .env — it's not a secret).
    try:
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Numbers Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Numbers Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Numbers Desktop (Linux only, best-effort).
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Numbers Local Signing"
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Numbers-managed Node on PATH so npm's child scripts (which
            ...
            if not source_mode:
                ...
                # holds Numbers.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Numbers.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Numbers.exe, close any")
                    print("  running Numbers desktop window and retry.")
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Numbers is
                ...
                # success on a Numbers.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Numbers shows up
    ...
    if source_mode:
        print("→ Launching Numbers Desktop from source build...")
    ...
    print(f"→ Launching packaged Numbers Desktop: {' '.join(launch_command)}")
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Numbers installs Linux services in the
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        ...
        if self_locked:
            print(
                "  Numbers is still running from the launcher that needs "
                "replacing. Close other Numbers windows, restart from a "
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the Numbers Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Numbers Desktop / gateway / REPL).
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Numbers Desktop, exit other `hermes` REPLs, stop the "
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Numbers versions queued for our shims.

    Numbers used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
...
def cmd_update(args):
    """Update Numbers to the latest version.
    ...
    if getattr(args, "plan", False):
        ...
        # Numbers runtime across profiles, its supervisor, and its running
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Numbers {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: Numbers {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No numbers dashboard or serve processes running.")
        return 0

    print(f"{len(live)} numbers dashboard/serve process(es) running:")
...
def cmd_dashboard(args):
    ...
    # the same logs directory as every other Numbers surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        # NUMBERS deviation: the fork root has no root node_modules, so the
        # live `npm install` inside _build_web_ui fails. Instead of exiting,
        # fall back to the prebuilt dist that ships in the repo. Only hard-fail
        # when there is no usable dist to serve.
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=False):
            _prebuilt = PROJECT_ROOT / "hermes_cli" / "web_dist"
            if (_prebuilt / "index.html").exists():
                print("  ⚠ Web UI build unavailable — serving the prebuilt dashboard.")
                print(f"  → Using prebuilt dist at {_prebuilt}")
            else:
                print("✗ Web UI build failed and no prebuilt dist was found at:")
                print(f"    {_prebuilt}")
...
def cmd_logs(args):
    """View and filter Numbers log files."""
...
def cmd_console(args):
    """Open the safe Numbers command console."""
...
def cmd_acp(args):
    """Launch Numbers as an ACP server."""
    try:
...
def _advertise_agent_env() -> None:
    ...
    ``HERMES_AGENT`` is the Numbers-specific marker. setdefault: never
    clobber an outer harness (e.g. Numbers running inside another agent's
    terminal).
    """
...
def main():
    ...
    browser_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "default profile so Numbers can copy it — DESTRUCTIVE (unsaved tabs "
    ...
    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Numbers gateways)
    # =========================================================================
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Numbers shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Numbers app. `status` reports the driver's grant\n"
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Numbers",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Numbers session store "
    ...
    # for one release. The Numbers-Setup.exe success screen tells users to
