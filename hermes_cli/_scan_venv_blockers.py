...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    ``numbers update --yes --gateway``) — never gets the chance to run.
...
def _is_updater_owned_backend(pid: int, cmdline: str) -> bool:
    """Return True when *pid* is a Numbers backend the CLI updater can stop.
    ...
    them. ``numbers serve`` / ``numbers dashboard`` backends had no such
