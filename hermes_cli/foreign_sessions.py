...
role-alternation invariant Numbers enforces everywhere else:
...
def import_foreign_session(source: str, path, db=None) -> str:
    """Import one foreign session into the Numbers SessionDB.

    Returns the new Numbers session id.  The foreign file is only read.
