...
Diagnoses issues with Numbers setup.
"""
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Numbers containers"
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for Numbers-managed SQLite databases."""
...
def _report_database_journal_modes(
    ...
    try:
        ...
        check_warn(f"Could not list Numbers databases: {exc}")
        return
    exposed = []
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Numbers Relay plugin."""
...
def check_macos_tcc_grants() -> None:
    ...
    check_info(
        ...
        "fully quit & relaunch Numbers once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Numbers.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Numbers.app``
    is deliberately not probed: it is the separately-signed Numbers-Setup
    ...
    candidates = [p for p in release_dir.glob("mac*/Numbers.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Numbers will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and Numbers.app if you use Desktop), "
        "and restart them once. With Numbers' stable signing identities the "
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Numbers Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Numbers already refuses to enable WAL on fresh DBs.
        ...
        try:
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    ...
                    issues.append("Run 'hermes doctor --fix' or 'numbers setup' to migrate config")
            else:
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Numbers' own device-code flow — the Codex CLI is
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Numbers a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Numbers Persona\n\n"
                "<!-- Edit this file to customize how Numbers communicates. -->\n\n"
                "You are Numbers, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    if state_db_path.exists():
        ...
        # with a runaway WAL was previously invisible to every Numbers
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Numbers one.
                        check_info(
