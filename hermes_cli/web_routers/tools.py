...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `numbers tools` runs interactively. Without this,
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``numbers tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `numbers tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``numbers tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    the same store ``numbers tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``numbers tools post-setup <key>`` and the frontend tails the log via
