...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Numbers-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Numbers that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Numbers that doesn't ship
...
#
# Some vendors expose several Numbers provider slugs (one per endpoint /
...
# In-repo fallback for the model Numbers silently lands on when the user never
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Numbers lands on when none is selected.
            desc = "default"
        else:
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Numbers' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Numbers should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Numbers release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Numbers release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Numbers release.
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Numbers' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Numbers' native Anthropic adapter. The live catalog may
    ...
    *,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Numbers will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Numbers will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Numbers
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Numbers cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Numbers actually ships.  (#46850)
            #
