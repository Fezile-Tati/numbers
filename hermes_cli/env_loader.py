"""Helpers for loading Numbers .env files consistently across entrypoints."""
...
# Used by setup / `numbers model` flows to label detected credentials so
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Numbers env-var keys.
...
# Behavioral routing keys a parent Numbers process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Numbers keys absent from ``.env``.
    ...
    parent Numbers process injects and that silently change *which provider
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `numbers setup` (or edit the "
...
def load_hermes_dotenv(
    *,
    ...
    """Load Numbers environment files with user config taking precedence.
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Numbers keys
    ...
    # 2. A fresh ``numbers update`` retry just completed a deferred dependency
    ...
    # `numbers setup` before the user switched terminal.backend in config.yaml)
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Numbers reads
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `numbers model`
