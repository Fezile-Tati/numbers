"""
Interactive setup wizard for Numbers.
...
# display_hermes_home imported lazily at call sites (stale-module safety during numbers update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Numbers Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Numbers using environment variables or config commands:")
    print_info("  numbers config set model.provider custom")
    print_info("  numbers config set model.base_url http://localhost:8080/v1")
    print_info("  numbers config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'numbers setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'numbers setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Numbers cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    numbers model            (pick any provider/model)")
        print_info("    numbers setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'numbers setup' to configure"))
    ...
    # Video generation — opt-in via `numbers tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'numbers setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'numbers setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'numbers tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'numbers setup terminal'"))
    ...
    # Spotify (OAuth via numbers auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'numbers setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('numbers setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('numbers setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('numbers setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('numbers setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('numbers setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('numbers config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('numbers config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('numbers config set <key> <value>', Colors.GREEN)}")
...
# between `numbers tools` and `numbers setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``numbers model``)
    ...
    provider added to ``numbers model`` is automatically available here.
    ...
    # Delegate to the shared numbers model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: numbers model")
    ...
    # on demand via `numbers auth add`, `numbers setup` vision, and
    # `numbers setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``numbers model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Numbers auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via numbers setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'numbers setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Numbers runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `numbers setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `numbers setup terminal`.
        ...
        try:
            ...
            # which() misses the uv Numbers installed. Bootstrapping one is
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `numbers setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Numbers delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Numbers to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  numbers config edit")
    print_info("   Open config in your editor:  numbers config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Numbers from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'numbers setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     numbers config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `hermes import` or `numbers setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `numbers setup tools` and `numbers tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Numbers profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and Numbers.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Numbers to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Numbers at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Numbers at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Numbers at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Numbers at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Numbers equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Numbers config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Numbers.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Numbers's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Numbers can preview what would be imported before making any changes.")
    print()
    ...
    # Execute the migration — overwrite=False so existing Numbers configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Numbers config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Numbers (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``numbers setup --portal`` and ``numbers portal``. This is the
    ...
    Numbers session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``numbers setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``numbers model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``numbers portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Numbers Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `numbers portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `numbers portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `numbers portal`: %s", exc)
        print()
    ...
    print_info("  Run `numbers portal info` to inspect routing.")
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``numbers model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      numbers setup           — full or quick (auto-detected)
      numbers setup model     — just model/provider
      numbers setup tts       — just text-to-speech
      numbers setup terminal  — just terminal backend
      numbers setup gateway   — just messaging platforms
      numbers setup tools     — just tool configuration
      numbers setup telemetry — just local shared metrics
      numbers setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Numbers Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Numbers Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Numbers installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Numbers configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'numbers setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Numbers?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'numbers config edit'")
    ...
    # Tune later with `numbers setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``numbers setup <section>``
    or switch providers with ``numbers model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'numbers setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    numbers setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  numbers setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Numbers.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Numbers update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    configuring, and troubleshooting Numbers) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``numbers tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``numbers setup agent`` /
    ``numbers config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    print_info("can help you drive and configure Numbers itself.")
    print()
    ...
    if path == 0:
        ...
        # `numbers update` runs don't re-inject them. Essential skills (the
        ...
        print_info("  Enable tools:        numbers tools")
        print_info("  Seed skills:         numbers skills opt-in --sync")
        print_info("  Add MCP servers:     hermes mcp add")
        print_info("  Enable plugins:      numbers plugins")
        print_info("  Tune agent settings: numbers setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`numbers update` runs from re-injecting them. Opt back in any")
            print_info("time with `numbers skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `numbers tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `numbers plugins list` / `numbers plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `numbers plugins`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   numbers tools")
    print_info("  Seed skills:         numbers skills opt-in --sync")
    print_info("  Add MCP servers:     hermes mcp add")
    print_info("  Tune agent settings: numbers setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'numbers setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Numbers to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'numbers setup gateway'.")
