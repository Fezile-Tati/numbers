...
#: managed binary dir (the default Numbers root's ``bin``, next to uv.exe)
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    the managed binary dir — the default Numbers root's ``bin``
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    ...
    # Per-machine anchor: the DEFAULT Numbers root, not get_hermes_home() —
