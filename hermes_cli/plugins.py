...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other Numbers loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Numbers' shipped guidance lives in the evidence-based
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Numbers home."""
...
# Highest manifest schema version this Numbers understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Numbers "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `numbers plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    # lookups and by ``numbers plugins list``. For a flat plugin at
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Numbers never
    ...
    # (``numbers plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Numbers paths, traversal syntax, and the security-
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Numbers config writers still retain their existing
    ...
    def profile_name(self) -> str:
        """Return the active Numbers profile name (e.g. ``"default"``).
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Numbers core) are trusted by default —
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        ``~/.hermes/.env`` loads, before Numbers reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Numbers' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Numbers' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``numbers model → Configure auxiliary models`` picker
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # contributed, so `numbers plugins list` still attributes them once the
        # full plugin loads.
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Numbers plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Numbers core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Numbers core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Numbers Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # invocation — including plain `numbers chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Numbers ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `numbers plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Numbers process (e.g. a pip
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``numbers plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`numbers plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``numbers tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `numbers plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Numbers validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Numbers does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `numbers plugins list` would
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Numbers home (profile switch reusing a slug
...
# Keyed cache: resolved Numbers home -> PluginManager. Numbers supports
...
def _plugin_home_key() -> Path:
    ...
    can temporarily switch Numbers home (env var *or* the context-local
    ``set_hermes_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Numbers home instead of
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Numbers profile/home.
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``numbers tools`` TUI so plugin-provided toolsets appear
