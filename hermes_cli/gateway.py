...
# when hermes_constants is cached from a pre-update version during `numbers update`.
...
# — restart may fail", and `numbers update` can deadlock behind it.  The loop
...
def _scan_gateway_pids(
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``numbers update`` hung exactly there
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``numbers update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Numbers profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    *,
    ...
    """Return running gateway PIDs mapped to Numbers profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def find_windows_gateway_services(
    *,
    ...
    validated profile gateway PID comes from Numbers's own PID file, and its
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``numbers update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Numbers's detached watcher as well would escape the
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``numbers update`` users
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Numbers Docker image — that one runs s6-overlay as
...
SERVICE_DESCRIPTION = "Numbers Gateway - Messaging Platform Integration"
...
def _profile_arg(hermes_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Numbers root to compare against. Used when
...
# Legacy service names from older Numbers installs that predate the
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Numbers gateway units.

    Detects unit files installed by older Numbers versions that used a
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Numbers gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Numbers gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Numbers gateway unit(s) detected from an older install:")
...
def remove_legacy_hermes_units(
    ...
    """Stop, disable, and remove legacy Numbers gateway unit files.
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Numbers gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Numbers gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``numbers setup`` (end of wizard)
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Numbers often sets ``HOME`` to a profile-scoped directory, but
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``numbers update`` later relocates/removes — the path
...
def _append_node_dir_for_service(
    ...
    The Numbers-managed Node under ``$HERMES_HOME/node`` goes first when it
    ...
    *hermes_root* is the Numbers home the unit will run against. System units
    ...
    # target Numbers home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Numbers home is known — probing here would stat the calling
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Numbers install"
    )
    return True
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``numbers update`` sees the flag on the live grandchild argv and hands
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Numbers install"
    )
    return True
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `numbers update`
...
def launchd_status(deep: bool = False):
    ...
    # Numbers PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Numbers PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Numbers install")
    else:
        print("⚠ Service definition is stale relative to the current Numbers install")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the Numbers gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'hermes' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/hermes/docker/entrypoint.sh before the Numbers command."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Numbers Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Numbers delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Numbers connects via the BlueBubbles REST API and receives",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Numbers will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``numbers setup gateway`` without needing the gateway to be running.
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Numbers will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Numbers will store the returned account_id/token in ~/.hermes/.env."
    )
    print_info(
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        # Stop, disable, and remove legacy Numbers gateway unit files from
