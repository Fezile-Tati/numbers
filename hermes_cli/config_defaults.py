"""Default configuration data for Numbers.
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hermes-cli"],
    # SQLite journal mode used by every Numbers database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Numbers server processes.
    ...
    "agent": {
        ...
        # the Numbers-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Numbers
        ...
        # app, ACP) in a code workspace, Numbers adds a coding operating brief
    ...
    "terminal": {
        ...
        # Root directory for Numbers' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Numbers uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        ...
        # skipped. When empty, Numbers auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Numbers sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Numbers
        ...
        # bundled Numbers image, which drops to the `hermes` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `numbers tools` picker's
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes Numbers spawn
        ...
        # browser tools) runs on a Numbers-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Numbers' packaged Chromium. Only
        ...
        "camofox": {
            # When true, Numbers sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Numbers should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before Numbers applies
    ...
    # single read_file page exceeds these limits, Numbers truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Numbers' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); hermes =
                                      # Numbers' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Numbers' local
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Numbers appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # "Hey Numbers" hands-free wake word. Always-on, on-device hotword
    ...
    # response. If not, Numbers feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Numbers auto-pauses the goal and
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Numbers*" header.
    ...
    # replace Numbers' built-in platform hint for a single messaging platform
    ...
    "security": {
        ...
        # .cursorrules, project-local .numbers config) always require human
        ...
        "acked_advisories": [],
        # Allow Numbers to lazy-install opt-in backend packages from PyPI
    ...
    #
    # Core Numbers tools (terminal, read_file, write_file, patch,
    ...
    "model_catalog": {
        ...
        # next /model or `numbers model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Numbers provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # behavior: files must live under the Numbers cache, under
        ...
        # uploaded as native gateway attachments. Files inside the Numbers
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # Bitwarden due to NETWORK/TIMEOUT, Numbers may use this encrypted
    ...
    "computer_use": {
        ...
        # by default upstream. Numbers disables it for our users unless they
        # explicitly opt in here. When false (default), Numbers sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Numbers also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Numbers computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Numbers passes the
    ...
    # Numbers works exactly as before with `enabled: false`.
    "proxy": {
    ...
    # Numbers Desktop (Electron app) launch options. These only affect
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Numbers mints short-lived tokens from. Falls back to "
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Numbers auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Numbers' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Numbers, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Numbers API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Numbers API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Numbers API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
