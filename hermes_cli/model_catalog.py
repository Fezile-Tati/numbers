...
The Numbers docs site hosts a JSON manifest of curated models for providers
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Numbers silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`numbers update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``numbers update`` pulls the latest repo, so the freshly-pulled
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``numbers model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
