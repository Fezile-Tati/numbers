...
Outputs a compact, plain-text summary of the user's Numbers setup
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `numbers auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `numbers auth list` shows it (#42130).
