...
The provider list shown by ``numbers model`` (CLI/TUI) and the desktop Settings
...
the same universe ``numbers model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``numbers model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `numbers model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``numbers model`` universe.
    ...
    # Numbers overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        # Numbers overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
