"""``numbers worktree`` — audit and reclaim accumulated git worktrees/branches.
...
``hermes_cli/worktree_gc.py`` for the policy and shared invariants). Usage:
    ...
    numbers worktree list             # audit: verdict + reason per tree
    numbers worktree prune            # reap safe trees + merged branches
    numbers worktree prune --dry-run  # show the plan, change nothing
    numbers worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `numbers worktree prune`."
        )
