...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Numbers. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Numbers deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Numbers on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Numbers in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Numbers in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Numbers agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Numbers")
    ...
    """
    name = getattr(args, "name", None) or "Numbers"
    description = getattr(args, "description", None) or "Your Numbers agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Numbers app\n"
