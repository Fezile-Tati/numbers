"""Numbers update pipeline — extracted from ``hermes_cli/main.py``.
...
def _purge_stale_hermes_modules() -> None:
    """Evict every cached Numbers module after the checkout changed in-place.

    ``numbers update`` keeps running in the pre-pull Python process. The
    ...
    export. This is the class fix: drop EVERY cached module under the Numbers
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Numbers module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Numbers modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``numbers update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``numbers update`` runs in the PRE-pull Python process. After ``git pull``
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``numbers update`` historically
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'numbers config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'numbers config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'numbers config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: numbers config migrate")
        else:
            print()
            print("Skipped. Run 'numbers config migrate' later to configure.")
    else:
...
# Critical files that Numbers must be able to import immediately after an
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    Skipping is safe because Numbers pins its editable finder to a *static*
...
def _critical_module_import_failures(
    ...
    ``_venv_core_imports_healthy``): ``numbers update`` can be driven by a
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``numbers update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `numbers update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `numbers update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `numbers sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    numbers sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  numbers sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``numbers update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``numbers update`` invocations skip the block until a newer
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `numbers update`" advice with
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``numbers update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && numbers update")
    print(
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Numbers launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["hermes.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Numbers")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `numbers update`.")
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Numbers runtime."""
    ...
    print(
        "  Rebuild the Numbers venv with a uv-managed Python, restart Numbers, "
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Numbers process the updater's drain
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not sqlite_runtime_ok:
            print(
                ...
                "update` again; if SQLite is unchanged, rebuild the Numbers venv "
                "with a uv-managed Python, restart Numbers, then verify with "
                "`hermes doctor`."
            )
    else:
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Numbers by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `numbers update "
            f"--branch {branch}`, or update against main with `numbers update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    # Pull the ZIP from the SAME repo `numbers update` tracks (origin = our
    # fork), never the hardcoded NousResearch upstream — otherwise a git-I/O
    # failure would silently overlay raw Hermes over a branded install.
    _zip_git_cmd = ["git"]
    if sys.platform == "win32":
        _zip_git_cmd = ["git", "-c", "windows.appendAtomically=false"]
    zip_url = _origin_archive_zip_url(_zip_git_cmd, _m().PROJECT_ROOT, branch)
    ...
    try:
        ...
        try:
            for item in entries:
                ...
                # Numbers.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `numbers update` to retry; if the agent won't start, "
    ...
    if not import_ok:
        ...
        print("  Re-run `numbers update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: numbers skills list-modified  "
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``numbers update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the Numbers agent unexecutable.")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Numbers behaves unexpectedly.")
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Numbers couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Numbers couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Numbers behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Numbers couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Numbers couldn't drop "
...
# The Numbers fork's archive base, used as the ZIP-fallback source when the
# origin remote cannot be read. Keep in sync with the `origin` remote shipped
# by the installer.
FORK_ARCHIVE_REPO = "https://github.com/Fezile-Tati/numbers"


def _numbers_maintainer_mode() -> bool:
    """True only when this checkout is a Numbers *maintainer* clone.

    Numbers is a downstream product fork. For shipped end-user installs the
    updater must ONLY ever fast-forward our own fork (``origin``); it must
    never pull from, prompt for, or fall back to the NousResearch upstream —
    doing so would overwrite the Numbers branding on the user's machine. A
    maintainer who intentionally merges upstream sets ``NUMBERS_MAINTAINER=1``
    to re-enable the upstream-sync + fork-prompt paths. Default (unset) =
    product mode.
    """
    import os as _os

    return _os.environ.get("NUMBERS_MAINTAINER", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _origin_archive_zip_url(git_cmd: list[str], cwd: Path, branch: str) -> str:
    """Build the GitHub source-ZIP URL for the ``origin`` remote (our fork).

    The Windows ZIP fallback must pull from the same repo ``numbers update``
    normally tracks (``origin`` = our fork), NOT the hardcoded NousResearch
    upstream — otherwise a git-I/O failure would silently overlay raw Hermes
    over a user's branded install. Reading ``git remote get-url origin`` is a
    metadata lookup, not tree file I/O, so it still works on the broken-git
    systems this fallback exists for. Falls back to the known fork URL.
    """
    url = (_get_origin_url(git_cmd, cwd) or "").strip()
    if url.endswith(".git"):
        url = url[:-4]
    url = url.rstrip("/")
    ...
    return f"{FORK_ARCHIVE_REPO}/archive/refs/heads/{branch}.zip"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    # Numbers product installs never track the NousResearch upstream (see
    # _numbers_maintainer_mode), so never prompt to add it — the prompt could
    # lead a user to switch origin to the official repo and lose branding.
    if not _numbers_maintainer_mode():
        return True
    from hermes_constants import get_hermes_home
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    """
    # Numbers is a downstream product fork: end-user updates must fast-forward
    # our own fork (origin) only, never sync from NousResearch upstream — that
    # would overwrite Numbers branding on the user's machine. Maintainers opt
    # back in with NUMBERS_MAINTAINER=1. Returning False = "check never
    # happened", which the callers already handle gracefully.
    if not _numbers_maintainer_mode():
        return False
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Numbers repository.")
...
def _invalidate_update_cache():
    ...
    ``numbers update``, every profile is now current.
    """
    homes = []
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `numbers update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `numbers update` or `hermes gateway restart`.",
            file=stream,
        )
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``numbers update``, finish a skipped restart.
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close Numbers Desktop, exit any open `hermes` REPLs, and")
    ...
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `numbers update --force` if you've already")
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``numbers update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Numbers Desktop backend child, an unrelated ``hermes`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``numbers tools``."""
    try:
        from hermes_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Numbers Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``numbers tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Numbers Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Numbers Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``numbers update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `numbers update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `numbers update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Numbers' editable-install extras or
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `numbers update` is exactly the step
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    numbers update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `numbers update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `numbers update`.")
            failed = []
            if any(
    ...
    # Numbers profile using this checkout. Keep one per-checkout cache under the
    # shared Numbers root rather than rerunning npm once per named profile.
    ...
    # `numbers update` case. Synchronous and can block ~11s on a true cold
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `numbers update`.")
        return list(labels)
    ...
    # `numbers update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    ...
    During ``numbers update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``numbers update --check``: fetch and report without installing.
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``numbers update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Numbers — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    ...
    gain the ACP command on ``numbers update`` without a reinstall.  ACP hosts
    ...
    PATH, so those hosts report Numbers as not installed even when it is.
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        ...
        try:
            ...
            shim = (
                "#!/usr/bin/env bash\n"
                "# Numbers — ACP launcher (written by `numbers update`).\n"
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``numbers update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``numbers update`` on a current checkout prints
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/Numbers candidates.
...
def _hermes_holder_subcommand(cmdline: str) -> str | None:
    """The actual Numbers SUBCOMMAND a venv-holder argv runs, or None.
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``numbers dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Numbers processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Numbers backend (if the Desktop app is open, close it)",
        "dashboard": "  ← numbers dashboard (stop it: numbers dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    ...
    lines.append(
        "  Close the Numbers desktop app / other Numbers terminals, then re-run:"
    )
    lines.append("    numbers update")
    lines.append("  (or use `numbers update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``numbers update``
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``numbers update`` through its terminal tool.
    ...
    print("  Otherwise, run `numbers update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    Numbers spawner). Returns the full ledger entries so the relauncher can
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (numbers serve --host <ip> --port <port>)."
        )
    try:
        from hermes_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    the update with "Numbers is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Numbers backend (``hermes_cli.main`` + ``serve`` /
      ``dashboard``), and
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Numbers ``serve``/``dashboard`` backends safe to reap during a
    ...
    - Only Numbers backends (``hermes_cli.main`` + ``serve``/``dashboard``)
    ...
    for entry in pids:
        ...
        try:
            ...
            if not pid_is_hermes(
                ...
                logger.debug(
                    "Skipping taskkill of non-Numbers or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``numbers serve`` / ``numbers dashboard`` argv.
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating Numbers...")
    try:
...
def _recover_gateway_restart_after_abort(
    ...
    ``numbers update`` normally performs the fleet restart in the interpreter
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``numbers update`` never touched them, so installs created before the
...
def _portable_git_candidates() -> list:
    ...
    The Numbers-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped HERMES_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``numbers update`` must
...
def _locate_real_git() -> Optional[Path]:
    ...
    Numbers-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``numbers update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``numbers update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    returns ``True``.
    """
    # NUMBERS deviation: the desktop app is deprecated (CLI-only). Skip the
    # rebuild entirely so `numbers update` no longer prints
    # "Checking if desktop app needs rebuilding..." / "Desktop app up to date"
    # or spawns an Electron build. Returning True keeps the caller contract
    # (a False would withhold "✓ Update complete!"). The `numbers desktop`
    # command itself is intentionally left intact.
    return True
    ...
    #
    # Start the build subprocess with the Numbers-managed Node on PATH: when
    # `numbers update` runs inside the desktop updater chain (Desktop →
    # hermes-setup → numbers update), the shell PATH customizations are lost,
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    ``numbers update`` then dies mid-mutation inside ``uv pip install -e .``
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Numbers broken.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # running Numbers runtime, its supervisor, and its running code version —
    ...
    # (most commonly the Desktop app's `numbers serve` backend) will keep .pyd
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Numbers backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Numbers backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # re-check instead of dead-ending with "Numbers is still
        ...
        if _venv_holders:
            ...
            # `numbers serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Numbers backends are leaks,
            ...
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Numbers backend process(es) "
    ...
    # process by design (a CLI `numbers update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Numbers windows/gateways and re-run: numbers update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Numbers gateways, Desktop backends, or other "
        ...
        # every user who ran ``numbers update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``numbers update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``numbers update`` printed "✓ Code updated!" and
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && numbers update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``numbers update`` can catch up even when git is already up to date.
        ...
        if not import_ok:
            ...
            print("    Run `numbers update` again — if it persists, reinstall:")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Numbers re-prompts for permissions you already "
        ...
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Numbers on PATH without
        ...
        # (the default Numbers root's bin, next to the managed uv) and repair
        ...
        # the managed clone under the default Numbers root).
        try:
        ...
        # don't have it. Tying the refresh to ``numbers update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `numbers update`
        ...
        # When running as ``numbers update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached Numbers modules FIRST: the import below pulls
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `numbers update` restart it automatically, allow\n"
        ...
        # Warn if legacy Numbers gateway unit files are still installed.
        # When both hermes.service (from a pre-rename install) and the
        # current hermes-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `numbers update` surfaces the issue until the user migrates.
        try:
            from hermes_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Numbers gateway unit(s) detected:")
        ...
        print("  numbers model              # Select provider and model")
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``numbers update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
