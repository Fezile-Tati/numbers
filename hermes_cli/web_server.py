"""
Numbers — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m hermes_cli.main web          # Start on http://127.0.0.1:2149
...
app = FastAPI(title="Numbers", version=__version__, lifespan=_lifespan)
...
# GUARD: this MUST equal `web/src/lib/api.ts` `SESSION_HEADER`. The web client
# sends this exact request-header name; a mismatch 401s every /api/* call.
# Do NOT rebrand it — the tree-wide `\bHermes\b` on-screen rebrand once flipped
# this to "X-Numbers-Session-Token" (hyphens are word boundaries) while api.ts
# stayed "X-Hermes-Session-Token", which broke the whole dashboard. It is an
# internal HTTP header, not on-screen text; keep it in lockstep with the client.
_SESSION_HEADER_NAME = "X-Hermes-Session-Token"


# Numbers on-screen branding, injected into index.html as window.__NUMBERS_BRANDING__.
# These are the ONLY defaults; each is overridable by an env var so brand text,
# the org footer, and the docs source can change without editing or rebuilding
# the SPA (the update-safe seam). `wordmark` is the sidebar/header brand, `org`/
# `orgUrl` drive the footer link, `docs` selects the /docs behaviour ("pdf"
# renders a download card served by /api/numbers/docs.pdf; "iframe" keeps the
# built-in external docs iframe).
_NUMBERS_BRAND_DEFAULTS = {
    "name": "Numbers",
    "wordmark": "NUMBERS 21:4-9",
    "org": "Intersession",
    "orgUrl": "https://172.20.10.12:3000/",
    "docsMode": "pdf",
    "docsUrl": "/api/numbers/docs.pdf",
}


def _numbers_branding_json() -> str:
    """Return the JSON literal for ``window.__NUMBERS_BRANDING__``.

    Reads env overrides (``NUMBERS_BRAND_*`` / ``NUMBERS_DOCS_*``) so a deploy
    can rebrand without touching the SPA bundle. ``json.dumps`` produces a
    safe, HTML-script-embeddable literal (no lone ``</script>`` risk for these
    short values).
    """
    d = _NUMBERS_BRAND_DEFAULTS
    branding = {
        "name": os.environ.get("NUMBERS_BRAND_NAME", d["name"]),
        "wordmark": os.environ.get("NUMBERS_BRAND_WORDMARK", d["wordmark"]),
        "org": os.environ.get("NUMBERS_BRAND_ORG", d["org"]),
        "orgUrl": os.environ.get("NUMBERS_BRAND_ORG_URL", d["orgUrl"]),
        "docs": {
            "mode": os.environ.get("NUMBERS_DOCS_MODE", d["docsMode"]),
            "url": os.environ.get("NUMBERS_DOCS_URL", d["docsUrl"]),
        },
    }
    return json.dumps(branding)
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Numbers-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already Numbers' canonical browser-facing URL behind a
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Numbers (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Numbers provider slug — fine.
    ...
    1. Vendor-name → Numbers-provider mapping: when the provider string is
       not a known Numbers provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Numbers
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Numbers credential-store basenames (see
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the hermes-webui image sharing the Numbers source tree),
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Numbers root is actually /opt/data
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Numbers inside an xterm.js PTY. Browser
...
# install, minted on first read and persisted under the ROOT Numbers home
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Numbers Cloud
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    """Upload a redacted debug report + full logs to Intersession, return the link.

    NUMBERS replaces the upstream public-paste upload with an Intersession-native
    path: the SAME force-redacted bundle is POSTed to the hub's authenticated
    Angel endpoint (``/api/angel/v1/debug-report``) using the ``/sign-in`` agent
    token, stored in Postgres, and a private link is returned. Synchronous (run
    in a worker thread) so the dashboard renders the returned link directly. See
    ``numbers_ext/debug_share_intersession.py``.
    """
    from numbers_ext.debug_share_intersession import (
        DebugShareAuthError,
        DebugShareUploadError,
        share_debug_report,
    )
    ...
    except DebugShareAuthError as exc:
        # Not signed in / token rejected — the user must run /sign-in.
        raise HTTPException(status_code=401, detail=str(exc))
    except DebugShareUploadError as exc:
        # Hub offline or refused to store the bundle.
...
@app.get("/api/numbers/docs.pdf")
async def numbers_docs_pdf():
    """Serve the Numbers documentation PDF.

    The /docs dashboard page renders a Download button pointing here when
    branding.docs.mode == "pdf". The file lives in NUMBERS_HOME (outside the
    git checkout) so `numbers update` never wipes it and the owner can drop in
    a new PDF without rebuilding. The installer seeds it from
    numbers-dist/docs/. Env override: NUMBERS_DOCS_PDF_PATH.
    """
    pdf_path = os.environ.get("NUMBERS_DOCS_PDF_PATH")
    path = Path(pdf_path) if pdf_path else get_hermes_home() / "docs" / "Numbers-Documentation.pdf"
    if not path.is_file():
        return JSONResponse(
            {
                "error": "Documentation PDF not found. Place it at "
                f"{path} (or set NUMBERS_DOCS_PDF_PATH)."
            },
            status_code=404,
        )
    return FileResponse(
        ...
        filename="Numbers-Documentation.pdf",
    )
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Numbers updates are managed outside this dashboard in "
...
async def check_hermes_update(force: bool = False):
    """Report whether a Numbers update is available, without applying it.
    ...
    Returns:
        ...
        current_version: installed Numbers version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "Numbers is managed by Termux APT; run `pkg upgrade hermes-agent`."
        )
        return payload
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Numbers supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _dashboard_skew_restart_hint() -> str:
    ...
    if os.environ.get("HERMES_SERVE_HEADLESS") == "1":
        return (
            ...
            "(use Restart backend in Numbers Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Numbers process to load the new code "
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Numbers from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Numbers to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Numbers from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Numbers to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Numbers in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Numbers through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Numbers via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Numbers through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Numbers to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Numbers inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Numbers to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Numbers through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Numbers to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Numbers to Microsoft Teams chats via the Bot Framework.",
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Numbers.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Numbers from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Numbers over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Numbers through iMessage via Photon's managed Spectrum platform.",
    ...
    "simplex": {
        "description": "Talk to Numbers over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Numbers to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Numbers as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "whatsapp_cloud": {
        ...
        "description": "Use Numbers via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Numbers Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Numbers").strip() or "Numbers"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Numbers appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — Numbers-managed terminal PKCE
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Numbers PKCE ({_get_hermes_oauth_file() if _get_hermes_oauth_file else None})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Numbers even
    when they also have a separate Numbers-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Numbers never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    # server). Numbers previously reimplemented that subscription-OAuth PKCE
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Numbers, so the disconnect
    ...
    user sees exactly what executes, and Numbers then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # catalog entry) but, unlike other external providers, Numbers still OWNS
    # the credential it can show here: the Numbers-managed PKCE file
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Numbers — run the disconnect command to remove it."
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Numbers-managed PKCE file and auth-store entry.
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Numbers "
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `numbers auth remove`) can seed again.
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Numbers,
...
def _resolve_chat_argv(
    ...
    # Numbers TUI child: build via the single spawn-env factory (profile-home
...
# ---------------------------------------------------------------------------
# /api/console — safe Numbers Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Numbers CLI
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Numbers Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Numbers inside WSL2 to use the dashboard's /chat "
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Numbers Teal canvas before the SPA flips the configured user
    theme into place.
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # Numbers branding seam: the dashboard SPA + the numbers-branding plugin
        # read window.__NUMBERS_BRANDING__ for the wordmark, org footer, and docs
        # source. Injecting it here (a Numbers-owned server path) keeps all brand
        # text/URL changes as server config — no SPA source edits, no rebuild —
        # so future upstream merges never clash on branding.
        branding_js = f"window.__NUMBERS_BRANDING__={_numbers_branding_json()};"
        if gated:
            bootstrap_script = (
                f"<script>"
                f"{branding_js}"
        ...
        # Without this the SPA paints the default Numbers Teal canvas, then
    ...
    # (the MC Pages app), not the Numbers backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Numbers Teal",         "description": "Classic dark teal — the canonical Numbers look"},
    {"name": "default-large", "label": "Numbers Teal (Large)", "description": "Numbers Teal with bigger fonts and roomier spacing"},
    ...
    for plugins_root, source in search_dirs:
        ...
        for child in children:
            ...
            try:
                ...
                # Only advertise a plugin whose built frontend actually exists.
                # A manifest without a built ``entry`` (e.g. hermes-achievements
                # / kanban shipped without their dashboard/dist) would otherwise
                # be listed, and the SPA would 404 loading its JS/CSS.
                entry_rel = data.get("entry", "dist/index.js")
                if not (child / "dashboard" / entry_rel).exists():
                    _log.info(
                        "Dashboard plugin %s has no built entry (%s) — not advertising",
                        name, entry_rel,
                    )
                    continue
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'numbers serve' / 'numbers dashboard' backend or the Numbers gateway. "
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Numbers exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if headless:
                ...
                print(f"  Numbers backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Numbers Web UI → http://{host}:{actual_port}")
