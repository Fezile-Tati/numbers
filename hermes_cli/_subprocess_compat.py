...
Numbers is developed on Linux / macOS and tested natively on Windows too.
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Numbers' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Numbers shells out to git from many non-interactive contexts — MCP catalog
...
def _text_names_hermes(text: str) -> bool:
    """True when *text* names Numbers at a path-segment / token boundary.
...
def _process_command_is_hermes(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Numbers code."""
    try:
        import psutil
...
def pid_is_hermes(
    ...
    *,
    ...
    The PID must be valid, currently exist, and identify a Numbers process. When
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``numbers update`` on slow-WMI machines
