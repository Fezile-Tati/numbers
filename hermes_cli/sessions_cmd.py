"""``numbers sessions`` command — extracted from ``hermes_cli/main.py``.
...
#: Default age floor for `numbers sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`numbers sessions prune --never-active` — drop leaked/dead keyed rows.
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    numbers sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "list":
        ...
        # `numbers sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `numbers sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `numbers sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `numbers sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: numbers sessions pin <session_id>"
            )
            return
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`numbers sessions optimize` later to reclaim freed space.)")
