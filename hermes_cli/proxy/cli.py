...
def _print_aiohttp_missing() -> None:
    print(
        "hermes proxy requires aiohttp. Run `numbers setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"numbers auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Numbers proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Numbers proxy upstream adapters\n")
