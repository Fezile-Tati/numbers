...
# xAI's public API is OpenAI-compatible for the endpoints Numbers commonly
# uses. The Responses endpoint is included because Numbers' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Numbers-managed OAuth credentials."""

    auth_hint = "numbers auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`numbers auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`numbers auth reset xai-oauth` or re-authenticate with "
                    "`numbers auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `numbers auth add xai-oauth --type oauth`."
            )
