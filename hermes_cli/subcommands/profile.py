...
def build_profile_parser(subparsers, *, cmd_profile: Callable) -> None:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Numbers instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `numbers update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
    ...
    profile_export.add_argument(
        ...
        help="Output file (default: a managed profile-exports/<name>-<timestamp>.tar.gz "
             "under the default Numbers home)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Numbers profile distribution. SOURCE can be a git URL "
