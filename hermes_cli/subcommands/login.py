...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``hermes login`` was removed in favor of ``numbers auth`` / ``numbers model``
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ...
      gets pointed at ``numbers model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `numbers auth` to manage credentials, "
            "`numbers model` to select a provider, or `numbers setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `numbers model`",
    )
    login_parser.add_argument(
