...
def _add_server_runtime_args(parser) -> None:
    ...
    """
    parser.add_argument(
        "--port", type=int, default=2149, help="Port (default 2149, 0 for auto-assign by OS)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Numbers web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Numbers web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Numbers web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Numbers desktop app shells (<= 0.15.x) spawn the
    ...
    # showed "Numbers couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Numbers Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Numbers backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Numbers backend server — the JSON-RPC/WebSocket gateway the "
