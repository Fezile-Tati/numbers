...
``<root>`` is the **shared Numbers root** (the parent of any active
...
the profile's Numbers home but are otherwise isolated: a worker spawned
...
def kanban_home() -> Path:
    """Return the shared Numbers root that anchors the kanban board.
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Numbers treats as user-controlled-but-trusted on the
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Numbers' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Numbers-managed storage. A board ``default_workdir`` pointing at a
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Numbers checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Numbers treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Numbers creates/reuses that linked worktree. With
      no ``workspace_path``, Numbers anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Numbers uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Numbers
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Numbers profile.
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Numbers ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. Numbers
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Numbers profile.
        # `_default_spawn` invokes ``hermes -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Numbers
    ...
    # Auto-dispatch is enabled by default because Numbers bundles the
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Numbers CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Numbers shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Numbers executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Numbers is launched from a venv and the ``hermes`` shim is not on
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Numbers or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Numbers root exists
