"""Numbers-managed uv and Python runtime repair.

Numbers owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
The Python backing the install is different: it is shared by every Numbers
...
def managed_uv_path() -> Path:
    """Return the path where Numbers keeps *its* uv binary.
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Numbers-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Numbers provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Numbers keeps databases "
            "out of WAL mode on this SQLite build. The next `numbers update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``numbers update`` runs
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``numbers update`` daily;
...
def update_managed_uv(
    *,
    ...
    Call this during ``numbers update`` so the managed copy stays current.
...
def _reload_hermes_constants():
    ...
    ``numbers update`` imports ``hermes_constants`` from the OLD checkout,
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Numbers generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `numbers update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    ...
    if holders:
        ...
        return True, f"other Numbers processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    ancestors on purpose — a CLI ``numbers update`` itself runs from the
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Numbers-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``numbers update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``numbers update`` repairs the runtime (issue class:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    if self_locked:
        ...
        print(
            "    Retrying `numbers update` from inside this venv cannot help: "
