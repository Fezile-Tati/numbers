...
def active_session_limit_message(
    ...
    return (
        f"Numbers is at the active session limit ({active_count}/{max_sessions})."
    ...
    with _FileLock(lock_path):
        try:
            ...
            return None, ActiveSessionRefusal(
                (
                    "Numbers could not read the active-session registry at "
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``numbers dashboard`` / ``serve``) never trips that
