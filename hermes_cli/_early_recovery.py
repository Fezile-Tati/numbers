...
#
# ``numbers update`` renames the live ``hermes*.exe`` shims aside
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Numbers already vendors
...
def recover_if_needed(
    ...
    recovery marker from a prior ``numbers update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `numbers update`: CLI and Desktop retries preserve
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``numbers update`` (or the self-lock preflight, #83569) left the
