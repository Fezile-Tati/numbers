...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Numbers may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Numbers gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Numbers gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Numbers")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'numbers claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Numbers migration."""
    ...
    # ── Phase 2b: Pre-apply backup of the Numbers home ─────────
    ...
    if not no_backup:
        try:
            ...
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Numbers home."
            )
    ...
    # 'numbers claw cleanup' separately.
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        if skipped_keys:
            ...
            print_info("  numbers config set OPENROUTER_API_KEY sk-or-v1-...")
