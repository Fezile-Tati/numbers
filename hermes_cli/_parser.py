...
_EPILOGUE = """
Examples:
    numbers                        Start interactive chat
    numbers chat -q "Hello"        Single query mode
    numbers --tui                  Launch the modern TUI (or set display.interface: tui)
    numbers --cli                  Force the classic REPL (overrides display.interface: tui)
    numbers -c                     Resume the most recent session
    numbers -c "my project"        Resume a session by name (latest in lineage)
    numbers --resume <session_id>  Resume a specific session by ID
    numbers --resume latest        Resume the most recent session (same as -c)
    numbers --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    ...
    numbers -s numbers-agent-dev,github-auth
    numbers -w                     Start in isolated git worktree
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="numbers",
        description="Numbers - AI assistant with tool-calling capabilities",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore your Numbers config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Numbers",
    )
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore your Numbers config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Numbers itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
