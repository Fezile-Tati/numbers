...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    ...
    Prefer the real ``hermes`` executable (argv[0] or PATH). When Numbers
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # sys.executable is the interpreter actually running Numbers (the
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see Numbers' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Numbers\n"
        "GenericName=Numbers Desktop\n"
        "Comment=Launch Numbers Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Numbers\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Numbers desktop entry. Return its path.
