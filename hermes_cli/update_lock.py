"""Cross-process mutual exclusion for in-flight Numbers updates.

Three different surfaces can start an update of the same install tree:

* ``numbers update`` from a terminal,
...
once — so a dashboard-spawned ``numbers update`` and an installer-driven
...
then spawns ``numbers update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Numbers is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ancestry check those users get exit 2 ("Numbers is still running") on every
  GUI update forever, with no Numbers process actually running.
"""
...
# its own pid before spawning `numbers update` as a child stage. The parent
...
# to show "Numbers is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Numbers home (never the context-local profile override):
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``numbers update`` as a (grand)child, so a
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Numbers update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `numbers update` as a stage): we run under ITS claim
