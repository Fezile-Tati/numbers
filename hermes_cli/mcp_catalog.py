...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Numbers install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Numbers understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Numbers is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Numbers
        understands. Update Numbers to install this entry.
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `numbers auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`numbers auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Numbers session to load its tools.",
        Colors.GREEN,
    ))
