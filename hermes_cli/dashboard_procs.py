...
def _scan_dashboard_processes(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Numbers Desktop Electron app to protect its own
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Numbers Desktop (or become PPID-1
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    ...
    # When the Numbers Desktop Electron app spawns this dashboard as a
...
def _detect_concurrent_hermes_instances(
    ...
    The Numbers Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    ...
    #      hermes.exe sitting *under* a non-Numbers parent (e.g. a Numbers
...
def _hermes_home_dir() -> Path:
    """Resolved Numbers home (HERMES_HOME override or ~/.hermes)."""
