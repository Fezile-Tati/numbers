...
# tree generates llms.txt — the index every LLM (Numbers included, via the
# hermes-agent skill) reads to learn what Numbers can do — and
...
# Rust crates — currently just the Tauri bootstrap installer (Numbers-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``numbers serve`` backend, the Docker image)
