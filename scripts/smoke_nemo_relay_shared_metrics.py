"""Run a real Numbers CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_hermes_executable(hermes_repo: Path) -> Path:
    ...
    raise SystemExit(
        "Numbers executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Numbers source checkout containing .venv/bin/hermes",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Numbers development environment requires jsonschema"
        ) from exc
...
def main() -> int:
    ...
    if result.returncode != 0:
        raise AssertionError(
            f"Numbers exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Numbers model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Numbers did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Numbers did not print the mock model response")
    ...
    print("Numbers -> NeMo Relay shared-metrics smoke test passed")
