...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Numbers-Session-Id`` API header) and are otherwise interpolated raw
...
class AIAgent:
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``numbers chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``numbers chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Numbers against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Numbers can't reach the model provider. You may be offline. "
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        # untrusted input (X-Numbers-Session-Id header) and must not escape
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than Numbers' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal Numbers model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Numbers-authored content from later user hand-edits.
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another Numbers process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Numbers process was writing to the state "
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Numbers YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``numbers auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        ...
        A Settings save (desktop ``PUT /api/env``, ``numbers setup``) updates
    ...
    def run_conversation(
        ...
        try:
            # Serialize the full load -> run -> flush region across Numbers
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Numbers process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Numbers process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Numbers process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Numbers process kept this session busy too "
