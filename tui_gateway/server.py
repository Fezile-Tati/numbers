...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Numbers' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Numbers-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Numbers agent → needs provider credentials.
        ...
        # Prepend the Numbers venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Numbers-managed CLIs
...
_SESSION_OWNERSHIP_UNAVAILABLE = (
    "Numbers could not safely reserve this session. Try again."
)
...
# A `numbers serve` killed mid-update used to lose every un-flushed in-memory
...
def install_exit_flush_signal_handlers() -> bool:
    ...
    Called by ``numbers serve`` / dashboard startup before uvicorn takes over
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``numbers dashboard`` was launched) instead of the configured
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Numbers Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Numbers Desktop app and start a new "
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``numbers config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Numbers activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    the same backend told the model it was "chatting inside the Numbers desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    # Coding posture (base Numbers): with no explicit pin, collapse to the
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Numbers window, and answers
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Numbers has to support both the legacy 8-row petdex atlas and the current
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero Numbers sessions still appear in the sidebar (#81723). Mirrors the
...
def _discover_repos_payload(
    ...
    repos even with zero numbers sessions. Session-derived roots cover repos
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Numbers sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Numbers
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `hermes` is interactive — use `/numbers chat -q …` or run `hermes` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`numbers setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`hermes gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`numbers sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`numbers config edit` needs $EDITOR in a real terminal"
    return None
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Numbers" if role == "assistant" else role.title()
