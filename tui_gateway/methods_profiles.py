...
def _(rid, params: dict) -> dict:
    """List Numbers profiles (name, path, model, description, skill count).
...
def _(rid, params: dict) -> dict:
    ...
    ``numbers setup`` in that flow to recover. A profile spawned as an
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Toolsets: the same filtered universe the `numbers tools`
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `numbers tools` / Settings is where
