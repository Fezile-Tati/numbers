...
export function HermesConsoleModal({ open, onClose }: HermesConsoleModalProps) {
  ...
  const promptRef = useRef("numbers> ");
  const inputPromptRef = useRef("numbers> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "numbers> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to NUMBERS Console...\x1b[0m");
