...
export default function SystemPage() {
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_hermes === false) {
      showToast(
        "Numbers updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
      ...
      <ConfirmDialog
        ...
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'numbers update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'numbers update' (${updateInfo?.update_command ?? "numbers update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Numbers</div>
                <div className="flex items-center gap-2">
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <ConfirmDialog
              ...
              title="Restore full Numbers backup?"
              description={`This will overwrite your current Numbers configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    Uploads a redacted system-info report + logs to Intersession
                    and returns a private link to share with support. Requires
                    sign-in; reports auto-delete after their retention window.
                  </span>
                </div>
              </div>
              <Button
