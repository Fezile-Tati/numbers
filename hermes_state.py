#!/usr/bin/env python3
"""
SQLite State Store for Numbers.
...
#
# The ceilings above bound Numbers's SQLite descriptors, which is only ever part
...
def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Numbers root for the guard.
...
#: is OURS: it declares "this process tree is running under Numbers test
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Numbers home *root*.
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Numbers root {root}). "
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``numbers sessions optimize`` (FTS merge + VACUUM)
...
def _wal_reset_repair_hint() -> str:
    ...
    ``numbers update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Numbers-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Numbers"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Numbers-managed install (uv-managed venv)
...
# threads inside ONE interpreter, yet a normal Numbers host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``numbers serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Numbers's fallback on
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Numbers home tree (``get_hermes_home()``) — the safe repair scope:
      Numbers owns those files, and the OS makes ``chmod`` fail on files the
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Numbers needs read-write access to open the database. "
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Numbers deliberately runs ``state.db`` in
...
# next ``numbers sessions optimize-storage`` on a capable host).
#
...
_HERMES_CMDLINE_MARKERS = ("hermes_cli.main", "hermes_cli/main", "numbers serve",
                           "hermes-agent", "hermes gateway", "numbers chat")


def _looks_like_hermes(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Numbers process?
    ...
    We only flag processes that look like Numbers, not every system daemon.
    """
    lower = cmdline.lower()
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Numbers
    ...
    # `numbers update` leaves mixed-version processes sharing the DB until
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `numbers update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `numbers sessions optimize-storage` on a host "
        ...
        while True:
            ...
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Numbers process held the "
    ...
    def _db_file_was_replaced(self) -> bool:
        ...
        if recorded_app:
            ...
            # replace. A copied Numbers DB that minted its own id is nonzero.
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Numbers process —
                        # only flag uninspectable holders that look like
                        # another Numbers instance, not every system daemon.
    ...
    #
    # `optimize_fts_storage()` (the `numbers sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`numbers sessions optimize-storage`) ──
    #
    ...
    # offline repair path behind ``numbers sessions repair-routing``.
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``numbers serve``, desktop WS sidecar), each backend registers a row
    ...
    def latest_conversation_boundary(
        ...
        uses — never the key alone.  ``X-Numbers-Session-Key`` accepts any
    ...
    def delete_session_if_empty(
        ...
        and ``numbers sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Numbers, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Numbers session.

        A Numbers session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Numbers session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `numbers sessions vacuum` runs in a transient
